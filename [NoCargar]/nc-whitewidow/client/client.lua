...
--// PickUp Weed \\--
RegisterNetEvent('nc-whitewidow:PickUpWeed', function()         
	...
	QBCore.Functions.Progressbar("search_register", "Picking Up Weed...", 11500, false, true, {
		...
		local LockPick = exports['nc-lock']:StartLockPickCircle(circles, timer)
		if LockPick then		
		TriggerServerEvent('nc-whitewidow:pickUpWeed')
		end
	end, function()
		ClearPedTasks(GetPlayerPed(-1))
	end)
...
--// PickUp Weed Skunk \\--
RegisterNetEvent('nc-whitewidow:PickUpWeedSkunk', function()         
	loadAnimDict("anim@amb@business@weed@weed_inspecting_lo_med_hi@")
	TaskPlayAnim(PlayerPedId(), 'anim@amb@business@weed@weed_inspecting_lo_med_hi@', 'weed_spraybottle_crouch_spraying_02_inspector', 8.0, -8, -1, 16, 0, 0, 0, 0, false, false, false)
	QBCore.Functions.Progressbar("search_register", "Picking Up Skunk...", 11500, false, true, {
		disableMovement = true,
		disableCarMovement = true,
		disableMouse = false,
		disableCombat = true,
		...
		local LockPick = exports['nc-lock']:StartLockPickCircle(circles, timer)
		if LockPick then	
		TriggerServerEvent('nc-whitewidow:pickUpSkunk')
		end
	end, function()
		ClearPedTasks(GetPlayerPed(-1))
	end)
...
--// PickUp Weed Ak47 \\--
RegisterNetEvent('nc-whitewidow:PickUpWeedAk47', function()         
	loadAnimDict("anim@amb@business@weed@weed_inspecting_lo_med_hi@")
	TaskPlayAnim(PlayerPedId(), 'anim@amb@business@weed@weed_inspecting_lo_med_hi@', 'weed_spraybottle_crouch_spraying_02_inspector', 8.0, -8, -1, 16, 0, 0, 0, 0, false, false, false)
	QBCore.Functions.Progressbar("search_register", "Picking Up AK-47...", 11500, false, true, {
		disableMovement = true,
		disableCarMovement = true,
		disableMouse = false,
		disableCombat = true,
		...
		local LockPick = exports['nc-lock']:StartLockPickCircle(circles, timer)
		if LockPick then		
		TriggerServerEvent('nc-whitewidow:pickUpAk47')
		end
	end, function()
		ClearPedTasks(GetPlayerPed(-1))
	end)
...
--// PickUp Weed Og-Kush \\--
RegisterNetEvent('nc-whitewidow:PickUpWeedOgKush', function()         
	loadAnimDict("anim@amb@business@weed@weed_inspecting_lo_med_hi@")
	TaskPlayAnim(PlayerPedId(), 'anim@amb@business@weed@weed_inspecting_lo_med_hi@', 'weed_spraybottle_crouch_spraying_02_inspector', 8.0, -8, -1, 16, 0, 0, 0, 0, false, false, false)
	QBCore.Functions.Progressbar("search_register", "Picking Up Og-Kush...", 11500, false, true, {
		disableMovement = true,
		disableCarMovement = true,
		disableMouse = false,
		disableCombat = true,
		...
		local LockPick = exports['nc-lock']:StartLockPickCircle(circles, timer)
		if LockPick then		
		TriggerServerEvent('nc-whitewidow:pickUpOgKush')
		end
	end, function()
		ClearPedTasks(GetPlayerPed(-1))
	end)
...
--// Trey \\--
RegisterNetEvent("nc-whitewidow:OpenPlate", function()
...
--// Grab Cola \\--
RegisterNetEvent('nc-whitewidow:Client:GrabCola', function()
	TriggerServerEvent("nc-whitewidow:GrabCola")
end)

--// Grab Snack \\--
RegisterNetEvent('nc-whitewidow:Client:GrabSnack', function()
	TriggerServerEvent("nc-whitewidow:GrabSnack")
end)

--// Grab Water \\--
RegisterNetEvent('nc-whitewidow:Client:GrabWater', function()
	TriggerServerEvent("nc-whitewidow:GrabWater")
end)

--// Process Cannabis \\--
RegisterNetEvent('nc-whitewidow:Client:ProcessCannabis', function()
	QBCore.Functions.TriggerCallback('nc-whitewidow:HasItemProcessCannabis', function(HasItems)
		if HasItems then
			ProcessWeedCannabis()
		else
...
--// Process Weed Skunk \\--
RegisterNetEvent('nc-whitewidow:Client:ProcessSkunk', function()
	QBCore.Functions.TriggerCallback('nc-whitewidow:HasItemProcessSkunk', function(HasItems)
		if HasItems then
			ProcessWeedSkunk()
		else
...
--// Process Weed Ak47 \\--
RegisterNetEvent('nc-whitewidow:Client:ProcessAk47', function()
	QBCore.Functions.TriggerCallback('nc-whitewidow:HasItemProcessAk47', function(HasItems)
		if HasItems then
			ProcessWeedAk47()
		else
...
RegisterNetEvent('nc-whitewidow:Client:ProcessOgKush', function()
	QBCore.Functions.TriggerCallback('nc-whitewidow:HasItemProcessOgKush', function(HasItems)
		if HasItems then
			ProcessWeedOgKush()
		else
...
--// Process Joints \\--
RegisterNetEvent('nc-whitewidow:Client:Joints', function()
	QBCore.Functions.TriggerCallback('nc-whitewidow:CheckForItemsJoint', function(HasItems)
		if HasItems then
			ProcessJoints()
		else
...
--// Shop \\--
RegisterNetEvent('nc-whitewidowShop:Open', function()
    TriggerServerEvent("inventory:server:OpenInventory", "shop", "WhiteWidow Shop", Config.Items)
end)

--// Stash \\--
RegisterNetEvent('nc-whitewidowShop:Stash', function()
...
--// Duty \\--
RegisterNetEvent('nc-whitewidowDuty:On', function()
	onDuty = not onDuty
	TriggerServerEvent("QBCore:ToggleDuty")
end)

--// Bong \\--
RegisterNetEvent("nc-whitewidow:Client:Bong")
AddEventHandler("nc-whitewidow:Client:Bong", function()
	QBCore.Functions.TriggerCallback('nc-whitewidow:HasItemSmokeBong', function(HasItems)
		if HasItems then
			QBCore.Functions.Progressbar("snort_coke", "Smoking Bong", 5000, false, true, {
				...
				anim = "bong_stage1",
				flags = 49,
				TriggerEvent("nc-whitewidow:SpawnBong")
			}, {}, {}, function() -- Done
...
RegisterNetEvent("nc-whitewidow:SpawnBong")
AddEventHandler("nc-whitewidow:SpawnBong", function()
...
RegisterNetEvent('nc-whitewidow:Process:Menu', function()
    TriggerEvent('qb-menu:client:openMenu', {
        {
        ...
        },
        {
			...
			params = {
				event = "nc-whitewidow:Client:ProcessCannabis",
			}
        },       
		{
			...
			params = {
				event = "nc-whitewidow:Client:ProcessSkunk",
			}
        },    
		{
			...
			params = {
				event = "nc-whitewidow:Client:ProcessAk47",
			}
        },   
		{
			...
			params = {
				event = "nc-whitewidow:Client:ProcessOgKush",
			}
        },   
		{
            header = "Close Menu",
...
RegisterNetEvent('nc-whitewidow:OpenBong:Menu', function()
    TriggerEvent('qb-menu:client:openMenu', {
        {
            header = "Smoke Bong",
            txt = "Smoke Bong But Dont Smoke To Much Otherwise You Will Feel High !",
			params = {
				event = "nc-whitewidow:Client:Bong",
			}
        },        
		{
            header = "Close Menu",
...
RegisterNetEvent('nc-whitewidowShop:Menu:Open', function()
    ...
    TriggerEvent('qb-menu:client:openMenu', {
		...
		},
		{
			...
			params = {
				event = "nc-whitewidow:Client:GrabCola",
			}
		},       
		{
			header = "Grab Snack",
			...
			params = {
				event = "nc-whitewidow:Client:GrabSnack",
			}
		},    
		{
			header = "Grab Water",
			...
			params = {
				event = "nc-whitewidow:Client:GrabWater",
			}
		},   
		{
            header = "Close Menu",
...
--// Joints Process Menu \\--
RegisterNetEvent('nc-whitewidow:ProcessJoints:Menu', function()
    TriggerEvent('qb-menu:client:openMenu', {
        {
        ...
        },
        {
			...
			params = {
				event = "nc-whitewidow:Client:Joints",
            }
        },       
		{
            header = "Close Menu",
...
CreateThread(function()
	local Player = QBCore.Functions.GetPlayerData()
		--// Shop Target \\--
		exports['qb-target']:AddBoxZone("WhiteWidowShopMenu", vector3(186.1, -243.3, 54.07), 1.0, 2.2, {
		...
		exports['qb-target']:AddBoxZone("WhiteWidowDrinksMenu", vector3(187.12597, -245.377, 53.808071), 0.8, 0.8, {
			...
			options = {
				{
					type = "client",
					event = "nc-whitewidowShop:Menu:Open",
					icon = "fa fa-shop",
		...
		exports['qb-target']:AddBoxZone("WhiteWidowPick1", vector3(170.10705, -244.7768, 50.367435), 1.0, 3, {
			...
			options = {
				{
					type = "client",
					event = "nc-whitewidow:PickUpWeed",
					icon = "fa fa-cannabis",
					label = "Pick Cannabis",
                    job = Config.Job,
				canInteract = function()
		...
		exports['qb-target']:AddBoxZone("WhiteWidowSkunkPick1", vector3(167.59107, -244.1141, 50.36737), 1.0, 3, {
			...
			options = {
				{
					type = "client",
					event = "nc-whitewidow:PickUpWeedSkunk",
					icon = "fa fa-cannabis",
					label = "Pick Skunk",
                    job = Config.Job,
				canInteract = function()
		...
		exports['qb-target']:AddBoxZone("WhiteWidowAk47Pick1", vector3(165.09664, -243.0975, 50.36737), 1.0, 3, {
			...
			options = {
				{
					type = "client",
					event = "nc-whitewidow:PickUpWeedAk47",
					icon = "fa fa-cannabis",
					label = "Pick AK-47",
					job = Config.Job,
				canInteract = function()
		...
		exports['qb-target']:AddBoxZone("WhiteWidowOgKushPick1", vector3(162.51042, -242.0645, 50.055507), 1.0, 3, {
			...
			options = {
				{
					type = "client",
					event = "nc-whitewidow:PickUpWeedOgKush",
					icon = "fa fa-cannabis",
					label = "Pick Og-Kush",
					job = Config.Job,
				canInteract = function()
		...
		exports['qb-target']:AddBoxZone("WhiteWidowProcess", vector3(165.55667, -235.6183, 50.681068), 1.0, 2.7, {
			...
			options = {
				{
					type = "client",
					event = "nc-whitewidow:Process:Menu",
					icon = "fa fa-smoking",
		...
		exports['qb-target']:AddBoxZone("WhiteWidowProcessJoints", vector3(164.46914, -233.2938, 50.274959), 1.0, 2.7, {
			...
			options = {
				{
					type = "client",
					event = "nc-whitewidow:ProcessJoints:Menu",
					icon = "fa fa-smoking",
		...
		for k, v in pairs(Config.BongLocations) do
			exports['qb-target']:AddBoxZone("BongLocation"..k, vector3(v.coords.x, v.coords.y, v.coords.z), v.poly1, v.poly2, {
				...
				maxZ = v.maxZ,
					...
					{
						type = "client",
						event = "nc-whitewidow:OpenBong:Menu",
						icon = "fa fa-bong",
						label = "Smoke Bong",
					}
				},
...
function ProcessWeedCannabis()
	...
	TriggerServerEvent('nc-whitewidow:ProcessCannabis')
	QBCore.Functions.Progressbar("search_register", "Processing...", 7500, false, true, {
		...
		ClearPedTasks(GetPlayerPed(-1))
		TriggerServerEvent('nc-whitewidow:ProcessCannabisAdd')
	end, function()
		ClearPedTasks(GetPlayerPed(-1))
	end)
end
...
function ProcessWeedSkunk()
	local playerPed = PlayerPedId()
	TaskStartScenarioInPlace(playerPed, "PROP_HUMAN_PARKING_METER", 0, true)
	TriggerServerEvent('nc-whitewidow:ProcessSkunk')
	QBCore.Functions.Progressbar("search_register", "Processing...", 7500, false, true, {
		disableMovement = true,
		disableCarMovement = true,
		disableMouse = false,
		...
		ClearPedTasks(GetPlayerPed(-1))
		TriggerServerEvent('nc-whitewidow:ProcessSkunkAdd')
	end, function()
		ClearPedTasks(GetPlayerPed(-1))
	end)
end
...
function ProcessWeedAk47()
	local playerPed = PlayerPedId()
	TaskStartScenarioInPlace(playerPed, "PROP_HUMAN_PARKING_METER", 0, true)
	TriggerServerEvent('nc-whitewidow:ProcessAk47')
	QBCore.Functions.Progressbar("search_register", "Processing...", 7500, false, true, {
		disableMovement = true,
		disableCarMovement = true,
		disableMouse = false,
		...
		TriggerServerEvent('nc-whitewidow:ProcessAddAk47')
		ClearPedTasks(GetPlayerPed(-1))
	end, function()
		ClearPedTasks(GetPlayerPed(-1))
	end)
...
function ProcessWeedOgKush()
	local playerPed = PlayerPedId()
	TaskStartScenarioInPlace(playerPed, "PROP_HUMAN_PARKING_METER", 0, true)
	TriggerServerEvent('nc-whitewidow:ProcessOgKush')
	QBCore.Functions.Progressbar("search_register", "Processing...", 7500, false, true, {
		disableMovement = true,
		disableCarMovement = true,
		disableMouse = false,
		...
		ClearPedTasks(GetPlayerPed(-1))
		TriggerServerEvent('nc-whitewidow:ProcessOgKushAdd')
	end, function()
		ClearPedTasks(GetPlayerPed(-1))
	end)
end
...
function ProcessJoints()
	TriggerServerEvent('nc-whitewidow:ProcessJoints')                                                       				
	QBCore.Functions.Progressbar("search_register", "Rolling Joints...", 7000, false, true, {
		...
		disableCombat = true,
	  	...
	  	ClearPedTasks(GetPlayerPed(-1))
	  	TriggerServerEvent('nc-whitewidow:ProcessJointsAdd')
   	end, function()
	  	ClearPedTasks(GetPlayerPed(-1))
   	end)  
end
