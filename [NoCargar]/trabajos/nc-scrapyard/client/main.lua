...
RegisterNetEvent("QBCore:Client:OnPlayerLoaded", function()
    TriggerServerEvent("nc-scrapyard:server:LoadVehicleList")
end)
							...
							if IsControlJustReleased(0, 38) then
								if GetPedInVehicleSeat(vehicle, -1) == PlayerPedId() then
									if IsVehicleValid(GetEntityModel(vehicle)) then
										...
										QBCore.Functions.TriggerCallback('nc-scrapyard:checkOwnerVehicle',function(retval)
											if retval then
...
RegisterNetEvent('nc-scapyard:client:setNewVehicles', function(vehicleList)
...
function ScrapVehicle(vehicle)
	...
	QBCore.Functions.Progressbar("scrap_vehicle", Lang:t('text.demolish_vehicle'), scrapTime, false, true, {
		...
		TriggerServerEvent("nc-scrapyard:server:ScrapVehicle", GetVehicleKey(GetEntityModel(vehicle)))
