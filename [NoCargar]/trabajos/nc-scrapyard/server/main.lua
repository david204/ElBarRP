...
RegisterNetEvent('nc-scrapyard:server:LoadVehicleList', function()
    local src = source
    TriggerClientEvent("nc-scapyard:client:setNewVehicles", src, Config.CurrentVehicles)
end)


QBCore.Functions.CreateCallback('nc-scrapyard:checkOwnerVehicle', function(_, cb, plate)
...
RegisterNetEvent('nc-scrapyard:server:ScrapVehicle', function(listKey)
    local src = source
    ...
    if Config.CurrentVehicles[listKey] ~= nil then
        ...
        TriggerClientEvent("nc-scapyard:client:setNewVehicles", -1, Config.CurrentVehicles)
    end
end)
