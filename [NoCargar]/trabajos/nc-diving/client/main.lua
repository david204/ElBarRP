...
local function callCops()
    ...
    if call == chance then
        TriggerServerEvent('nc-diving:server:CallCops', coords)
    end
end
...
local function takeCoral(coral)
    ...
    QBCore.Functions.Progressbar("take_coral", Lang:t("info.collecting_coral"), times * 1000, false, true, {
        ...
        TriggerServerEvent('nc-diving:server:TakeCoral', currentDivingLocation.area, coral, true)
        ClearPedTasks(ped)
    ...
    for k, v in pairs(Config.CoralLocations[currentDivingLocation.area].coords.Coral) do
        if Config.UseTarget then
            ...
            zones[k]:onPlayerInOut(function(inside)
                if inside then
                    currentArea = k
                    exports['nc-drawtext']:DrawText(Lang:t("info.collect_coral_dt"))
                else
                    currentArea = 0
                    exports['nc-drawtext']:HideText()
                end
            end)
        end
    end
...
local function sellCoral()
    ...
    QBCore.Functions.Progressbar("sell_coral_items", Lang:t("info.checking_pockets"), math.random(2000, 4000), false, true, {}, {}, {}, {}, function() -- Done
        ...
        TriggerServerEvent('nc-diving:server:SellCoral')
    ...
    for i = 1, #Config.SellLocations do
        ...
        if Config.UseTarget then
            ...
            zone:onPlayerInOut(function(inside)
                if inside then
                    inSellerZone = true
                    exports['nc-drawtext']:DrawText(Lang:t("info.sell_coral_dt"))
                else
                    inSellerZone = false
                    exports['nc-drawtext']:HideText()
                end
...
RegisterNetEvent('QBCore:Client:OnPlayerLoaded', function()
    QBCore.Functions.TriggerCallback('nc-diving:server:GetDivingConfig', function(config, area)
...
RegisterNetEvent('nc-diving:client:NewLocations', function()
    QBCore.Functions.TriggerCallback('nc-diving:server:GetDivingConfig', function(config, area)
        Config.CoralLocations = config
        setDivingLocation(area)
    end)
...
RegisterNetEvent('nc-diving:client:UpdateCoral', function(area, coral, bool)
    Config.CoralLocations[area].coords.Coral[coral].PickedUp = bool
end)

RegisterNetEvent('nc-diving:server:CallCops', function(coords, msg)
...
RegisterNetEvent('nc-diving:client:UseGear', function(bool)
    local ped = PlayerPedId()
    if bool then
        gearAnim()
        QBCore.Functions.Progressbar("equip_gear", Lang:t("info.put_suit"), 5000, false, true, {}, {}, {}, {}, function() -- Done
            ...
            TriggerServerEvent('nc-diving:server:RemoveGear')
            ClearPedTasks(ped)
        ...
        if currentGear.enabled then
            ...
            QBCore.Functions.Progressbar("remove_gear", Lang:t("info.pullout_suit"), 5000, false, true, {}, {}, {}, {}, function() -- Done
                ...
                TriggerServerEvent('nc-diving:server:GiveBackGear')
                ClearPedTasks(ped)
        ...
        if isLoggedIn then
            if currentArea ~= 0 then
                ...
                if IsControlJustPressed(0, 51) then -- E
                    ...
                    exports['nc-drawtext']:KeyPressed()
                    Wait(500)
                    exports['nc-drawtext']:HideText()
                    sleep = 3000
                end
...
----- Sell Fish
RegisterNetEvent('nc-diving:sellfish', function()
    sellCoral()
end)
