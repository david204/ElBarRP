...
RegisterNetEvent('nc-diving:server:CallCops', function(coords)
    for _, Player in pairs(QBCore.Functions.GetQBPlayers()) do
        if Player then
            if Player.PlayerData.job.name == "police" and Player.PlayerData.job.onduty then
                local msg = Lang:t("info.cop_msg")
                TriggerClientEvent('nc-diving:client:CallCops', Player.PlayerData.source, coords, msg)
                local alertData = {
...
RegisterNetEvent('nc-diving:server:SellCoral', function()
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent('nc-diving:server:TakeCoral', function(area, coral, bool)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player then return end
    ...
    if (Config.CoralLocations[area].TotalCoral - 1) == 0 then
        ...
        TriggerClientEvent('nc-diving:client:NewLocations', -1)
    else
    ...
    TriggerClientEvent('nc-diving:client:UpdateCoral', -1, area, coral, bool)
end)

RegisterNetEvent('nc-diving:server:RemoveGear', function()
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player then return end
...
RegisterNetEvent('nc-diving:server:GiveBackGear', function()
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player then return end
...
QBCore.Functions.CreateCallback('nc-diving:server:GetDivingConfig', function(_, cb)
...
QBCore.Functions.CreateUseableItem("diving_gear", function(source)
    TriggerClientEvent("nc-diving:client:UseGear", source, true)
end)

-- Commands

QBCore.Commands.Add("divingsuit", Lang:t("info.command_diving"), {}, false, function(source)
    TriggerClientEvent("nc-diving:client:UseGear", source, false)
end)
