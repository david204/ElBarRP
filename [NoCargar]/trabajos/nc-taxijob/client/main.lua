                ...
                if dist < 20 then
                    ...
                    if dist < 5 then
                        ...
                        if IsControlJustPressed(0, 38) then
                            ...
                            TriggerServerEvent('nc-taxi:server:NpcPay', meterData.currentFare)
                            meterActive = false
                            SendNUIMessage({
                                action = "resetMeter"
                            })
...
function TaxiGarage()
    ...
    for _, v in pairs(Config.AllowedVehicles) do
        vehicleMenu[#vehicleMenu+1] = {
            header = v.label,
            params = {
                event = "nc-taxi:client:TakeVehicle",
                args = {
                    model = v.model
                }
            }
...
RegisterNetEvent("nc-taxi:client:TakeVehicle", function(data)
...
-- Events
RegisterNetEvent('nc-taxi:client:DoTaxiNpc', function()
...
RegisterNetEvent('nc-taxi:client:toggleMeter', function()
    local ped = PlayerPedId()
...
RegisterNetEvent('nc-taxi:client:enableMeter', function()
    if meterIsOpen then
        SendNUIMessage({
            action = "toggleMeter"
        })
...
RegisterNetEvent('nc-taxi:client:toggleMuis', function()
    Wait(400)
    if meterIsOpen then
...
RegisterNetEvent('nc-taxijob:client:requestcab')
AddEventHandler('nc-taxijob:client:requestcab', function()
    TaxiGarage()
end)
        ...
        exports['qb-target']:SpawnPed({
            ...
            target = {
                options = {
                    {
                        type = "client",
                        event = "nc-taxijob:client:requestcab",
    ...
    zone:onPlayerInOut(function(isPlayerInside)
        if isPlayerInside then
            if whitelistedVehicle() and not isInsidePickupZone and not NpcData.NpcTaken then
                ...
                exports['textUi']:DrawTextUi('show', "[E] Call NPC", Config.DefaultTextLocation)
                callNpcPoly()
    ...
    delieveryZone:onPlayerInOut(function(isPlayerInside)
        if isPlayerInside then
            if whitelistedVehicle() and not isInsideDropZone and NpcData.NpcTaken then
                ...
                exports['textUi']:DrawTextUi('show', "[E] Drop off NPC", Config.DefaultTextLocation)
                dropNpcPoly()
                exports['textUi']:DrawTextUi('hide')
            end
        else
...
function setupCabParkingLocation()
    ...
    taxiParking:onPlayerInOut(function(isPlayerInside)
        if isPlayerInside and not Notified and Config.UseTarget then
            if whitelistedVehicle() then
                exports['textUi']:DrawTextUi('show', "[E] Vehicle Parking", Config.DefaultTextLocation)
                Notified = true
        ...
        if isPlayerInsideZone then
            if IsControlJustReleased(0, 38) then
                ...
                if IsPedInAnyVehicle(PlayerPedId(), false) then
                    ...
                    if meterIsOpen then
                        TriggerEvent('nc-taxi:client:toggleMeter')
                        meterActive = false
                    end
...
-- switched boss menu from nc-bossmenu to taxijob
CreateThread(function()
    while true do
        local sleep = 1000
