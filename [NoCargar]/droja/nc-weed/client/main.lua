...
RegisterNetEvent('nc-weed:client:getHousePlants', function(house)
    QBCore.Functions.TriggerCallback('nc-weed:server:getBuildingPlants', function(plants)
        currentHouse = house
                            ...
                            if plantData["plantStage"] ~= plantData["plantStats"]["highestStage"] then
                                ...
                                if IsControlJustPressed(0, 38) then
                                    QBCore.Functions.Progressbar("remove_weed_plant", "Harvesting Plant", 8000, false, true, {
                                        ...
                                        TriggerServerEvent('nc-weed:server:harvestPlant', currentHouse, amount, plantData["plantSort"]["name"], plantData["plantStats"]["plantId"])
                                    end, function() -- Cancel
                                        ClearPedTasks(ped)
                        ...
                        elseif plantData["plantStats"]["health"] == 0 then
                            ...
                            if IsControlJustPressed(0, 38) then
                                QBCore.Functions.Progressbar("remove_weed_plant", "Removing The Plant", 8000, false, true, {
                                    ...
                                    ClearPedTasks(ped)
                                    TriggerServerEvent('nc-weed:server:removeDeathPlant', currentHouse, plantData["plantStats"]["plantId"])
                                end, function() -- Cancel
                                    ClearPedTasks(ped)
                                    QBCore.Functions.Notify("Process Canceled", "error")
                                end)
...
RegisterNetEvent('nc-weed:client:leaveHouse', function()
    despawnHousePlants()
...
RegisterNetEvent('nc-weed:client:refreshHousePlants', function(house)
...
RegisterNetEvent('nc-weed:client:refreshPlantStats', function()
    if insideHouse then
        despawnHousePlants()
        SetTimeout(500, function()
            QBCore.Functions.TriggerCallback('nc-weed:server:getBuildingPlants', function(plants)
...
RegisterNetEvent('nc-weed:client:placePlant', function(type, item)
    local ped = PlayerPedId()
	...
	LocalPlayer.state:set("inv_busy", true, true)
		...
		LocalPlayer.state:set("inv_busy", false, true)				
                ...
                ClearPedTasks(ped)
                TriggerServerEvent('nc-weed:server:placePlant', json.encode(plantData["plantCoords"]), type, currentHouse)
                TriggerServerEvent('nc-weed:server:removeSeed', item.slot, type)
            end, function() -- Cancel
                ClearPedTasks(ped)
                QBCore.Functions.Notify("Process Canceled", "error")
		LocalPlayer.state:set("inv_busy", false, true)				
...
RegisterNetEvent('nc-weed:client:foodPlant', function(item)
    local plantData = {}
    if currentHouse ~= nil then
		...
		LocalPlayer.state:set("inv_busy", true, true)
			...
			LocalPlayer.state:set("inv_busy", false, true)
                        ...
                        TriggerServerEvent('nc-weed:server:foodPlant', currentHouse, newFood, plantData["plantSort"]["name"], plantData["plantStats"]["plantId"])
                    end, function() -- Cancel
                        ClearPedTasks(ped)
			LocalPlayer.state:set("inv_busy", false, true)					
                        QBCore.Functions.Notify("Process Canceled", "error")
