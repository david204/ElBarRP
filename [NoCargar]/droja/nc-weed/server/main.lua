...
QBCore.Functions.CreateCallback('nc-weed:server:getBuildingPlants', function(source, cb, building)
...
RegisterNetEvent('nc-weed:server:placePlant', function(coords, sort, currentHouse)
    ...
    TriggerClientEvent('nc-weed:client:refreshHousePlants', -1, currentHouse)
end)

RegisterNetEvent('nc-weed:server:removeDeathPlant', function(building, plantId)
    MySQL.Async.execute('DELETE FROM house_plants WHERE plantid = ? AND building = ?', {plantId, building})
    TriggerClientEvent('nc-weed:client:refreshHousePlants', -1, building)
end)
...
CreateThread(function()
    while true do
        ...
        TriggerClientEvent('nc-weed:client:refreshPlantStats', -1)
        Wait((60 * 1000) * 19.2)
    end
end)
...
QBCore.Functions.CreateUseableItem("weed_white-widow_seed", function(source, item)
    ...
    TriggerClientEvent('nc-weed:client:placePlant', source, 'white-widow', item)
end)

QBCore.Functions.CreateUseableItem("weed_skunk_seed", function(source, item)
    local Player = QBCore.Functions.GetPlayer(source)
    TriggerClientEvent('nc-weed:client:placePlant', source, 'skunk', item)
end)

QBCore.Functions.CreateUseableItem("weed_purple-haze_seed", function(source, item)
    local Player = QBCore.Functions.GetPlayer(source)
    TriggerClientEvent('nc-weed:client:placePlant', source, 'purple-haze', item)
end)

QBCore.Functions.CreateUseableItem("weed_og-kush_seed", function(source, item)
    local Player = QBCore.Functions.GetPlayer(source)
    TriggerClientEvent('nc-weed:client:placePlant', source, 'og-kush', item)
end)

QBCore.Functions.CreateUseableItem("weed_amnesia_seed", function(source, item)
    local Player = QBCore.Functions.GetPlayer(source)
    TriggerClientEvent('nc-weed:client:placePlant', source, 'amnesia', item)
end)

QBCore.Functions.CreateUseableItem("weed_ak47_seed", function(source, item)
    local Player = QBCore.Functions.GetPlayer(source)
    TriggerClientEvent('nc-weed:client:placePlant', source, 'ak47', item)
end)

QBCore.Functions.CreateUseableItem("weed_nutrition", function(source, item)
    local Player = QBCore.Functions.GetPlayer(source)
    TriggerClientEvent('nc-weed:client:foodPlant', source, item)
end)

RegisterServerEvent('nc-weed:server:removeSeed')
AddEventHandler('nc-weed:server:removeSeed', function(itemslot, seed)
    local Player = QBCore.Functions.GetPlayer(source)
    Player.Functions.RemoveItem(seed, 1, itemslot)
end)

RegisterNetEvent('nc-weed:server:harvestPlant', function(house, amount, plantName, plantId)
    local src = source
        ...
        if weedBag.amount >= sndAmount then
            if house ~= nil then
                ...
                if result[1] ~= nil then
                    ...
                    TriggerClientEvent('nc-weed:client:refreshHousePlants', -1, house)
                else
...
RegisterNetEvent('nc-weed:server:foodPlant', function(house, amount, plantName, plantId)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
