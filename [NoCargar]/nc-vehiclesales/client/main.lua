                ...
                if Config.UseTarget then
                    ...
                    EntityZones[i] = exports['qb-target']:AddTargetEntity(occasionVehicles[Zone][i].car, {
                        options = {
                            {
                                type = "client",
                                event = "nc-vehiclesales:client:OpenContract",
                                icon = "fas fa-car",
...
local function SellData(data, model)
    QBCore.Functions.TriggerCallback("nc-vehiclesales:server:CheckModelName",function(DataReturning)
        local vehicleData = {}
        ...
        TriggerServerEvent('nc-occasions:server:sellVehicle', data.price, vehicleData)
        ...
        while listen do
            if IsControlJustReleased(0, 38) then -- E
                if spot then
                    ...
                    TriggerEvent('nc-vehiclesales:client:OpenContract', data)
                else
                    if IsPedInAnyVehicle(PlayerPedId(), false) then
                        listen = false
                        TriggerEvent('nc-occasions:client:MainMenu')
                        --TriggerEvent('nc-vehiclesales:client:SellVehicle')
                    else
    ...
    for k, v in pairs(Config.Zones) do
        ...
        SellSpot:onPlayerInOut(function(isPointInside)
            if isPointInside and Zone ~= k then
                Zone = k
                QBCore.Functions.TriggerCallback('nc-occasions:server:getVehicles', function(vehicles)
...
RegisterNUICallback('buyVehicle', function(_, cb)
    TriggerServerEvent('nc-occasions:server:buyVehicle', CurrentVehicle)
    cb('ok')
end)

RegisterNUICallback('takeVehicleBack', function(_, cb)
    TriggerServerEvent('nc-occasions:server:ReturnVehicle', CurrentVehicle)
    cb('ok')
end)

-- Events

RegisterNetEvent('nc-occasions:client:BuyFinished', function(vehdata)
...
RegisterNetEvent('nc-occasions:client:SellBackCar', function()
    ...
    if IsPedInAnyVehicle(ped, false) then
        ...
        QBCore.Functions.TriggerCallback('nc-garages:server:checkVehicleOwner', function(owned, balance)
            if owned then
                if balance < 1 then
                    TriggerServerEvent('nc-occasions:server:sellVehicleBack', vehicleData)
...
RegisterNetEvent('nc-occasions:client:ReturnOwnedVehicle', function(vehdata)
    local vehmods = json.decode(vehdata.mods)
    DoScreenFadeOut(250)
    Wait(500)
    QBCore.Functions.TriggerCallback('QBCore:Server:SpawnVehicle', function(netId)
...
RegisterNetEvent('nc-occasion:client:refreshVehicles', function()
    if Zone then
        QBCore.Functions.TriggerCallback('nc-occasions:server:getVehicles', function(vehicles)
            despawnOccasionsVehicles()
            spawnOccasionsVehicles(vehicles)
...
RegisterNetEvent('nc-vehiclesales:client:SellVehicle', function()
...
RegisterNetEvent('nc-vehiclesales:client:OpenContract', function(data)
    CurrentVehicle = occasionVehicles[Zone][data.Contract]
    if CurrentVehicle then
        QBCore.Functions.TriggerCallback('nc-occasions:server:getSellerInformation', function(info)
            if info then
...
RegisterNetEvent('nc-occasions:client:MainMenu', function()
    local MainMenu = {
        {
            isMenuHeader = true,
        ...
        },
        {
            ...
            params = {
                event = 'nc-vehiclesales:client:SellVehicle',
            }
        },
        {
            ...
            params = {
                event = 'nc-occasions:client:SellBackCar',
            }
        }
    }
