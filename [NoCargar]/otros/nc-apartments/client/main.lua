    ...
    QBCore.Functions.TriggerCallback('apartments:GetApartmentOffset', function(offset)
        if offset == nil or offset == 0 then
            QBCore.Functions.TriggerCallback('apartments:GetApartmentOffsetNewOffset', function(newoffset)
                ...
                local data = exports['nc-interior']:CreateApartmentFurnished(coords)
                Wait(100)
                HouseObj = data[1]
                POIOffsets = data[2]
                InApartment = true
                ...
                Wait(100)
                TriggerServerEvent('nc-apartments:server:SetInsideMeta', house, apartmentId, true, false)
            ...
            TriggerEvent('nc-weathersync:client:DisableSync')
            Wait(100)
            TriggerServerEvent('nc-apartments:server:SetInsideMeta', house, apartmentId, true, true)
            TriggerServerEvent("InteractSound_SV:PlayOnSource", "houses_door_close", 0.1)
            TriggerServerEvent("QBCore:Server:SetMetaData", "currentapartment", CurrentApartment)
        end
        ...
        if new ~= nil then
            if new then
                TriggerEvent('nc-interior:client:SetNewState', true)
            else
                TriggerEvent('nc-interior:client:SetNewState', false)
            end
        else
            TriggerEvent('nc-interior:client:SetNewState', false)
        end
...
local function LeaveApartment(house)
    TriggerServerEvent("InteractSound_SV:PlayOnSource", "houses_door_open", 0.1)
    openHouseAnim()
    TriggerServerEvent("nc-apartments:returnBucket")
    DoScreenFadeOut(500)
    ...
    exports['nc-interior']:DespawnInterior(HouseObj, function()
        ...
        TriggerServerEvent('nc-apartments:server:SetInsideMeta', CurrentApartment, false)
...
RegisterNetEvent('apartments:client:setupSpawnUI', function(cData)
    QBCore.Functions.TriggerCallback('apartments:GetOwnedApartment', function(result)
        if result then
            TriggerEvent('nc-spawn:client:setupSpawns', cData, false, nil)
            TriggerEvent('nc-spawn:client:openUI', true)
            TriggerEvent("apartments:client:SetHomeBlip", result.type)
        else
            if Apartments.Starting then
                TriggerEvent('nc-spawn:client:setupSpawns', cData, true, Apartments.Locations)
                TriggerEvent('nc-spawn:client:openUI', true)
            else
                TriggerEvent('nc-spawn:client:setupSpawns', cData, false, nil)
                TriggerEvent('nc-spawn:client:openUI', true)
...
RegisterNetEvent('nc-apartments:client:LastLocationHouse', function(apartmentType, apartmentId)
