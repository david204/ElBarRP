...
CreateThread(function()
	for k,v in ipairs(Settings.BarberShops) do
	...
	for k,v in ipairs(Settings.ClothingShops) do
		local data = v
...
local inSpawn = false
AddEventHandler("nc-clothing:inSpawn", function(pInSpawn)
    inSpawn = pInSpawn
end)
...
function OpenMenu(name)
    ...
    if((oldPed.model ~= 1885233650 or oldPed.model ~= -1667301416) and (name == "clothesmenu" or name == "hatsmenu" or name == "tattoomenu" or name == "healmenu" or name == "barbermenu")) then isAllowed = true end
    if isAllowed then
        FreezePedCameraRotation(player, true)
        RefreshUI()
        EnableGUI(true, name)
...
function Save(save)
    if save then
        ...
        -- if firstChar then
        --     TriggerServerEvent("nc-characters:loadNewPlayer")
        -- end
    else
        LoadPed(oldPed)
    end
...
--    for _, item in pairs(Settings.BarberBlips) do
...
RegisterNetEvent('clothing:interact')
AddEventHandler('clothing:interact', function(Current)
    TriggerServerEvent("clothing:checkMoney", Current, StoreCost)
end)
...
AddEventHandler("nc-clothing:newChar",function()
    CreateThread(function()
        firstChar = true
...
RegisterNetEvent('nc-clothing:openClothing', function(pDontShowBarber, pShouldCost)
    if pShouldCost ~= nil and pShouldCost then
        currentPrice = priceWithTax.total
        priceWithTax.text = priceWithTax.text
        OpenMenu("clothesmenu", priceWithTax.text, currentPrice)
        startingMenu = false
    else
        currentPrice = 0
        OpenMenu("clothesmenu", '', 0)
        startingMenu = true
    end
...
RegisterNetEvent("nc-clothing:stealOutfit")
AddEventHandler("nc-clothing:stealOutfit", function(cheater)
    TriggerServerEvent("nc-clothing:letSteal", cheater, GetCurrentPed())
end)

RegisterNetEvent("nc-clothing:letSteal")
AddEventHandler("nc-clothing:letSteal", function(data)
    ForceOutfit = GetCurrentPed()
    LoadPed(data)
end)

RegisterNetEvent("nc-clothing:restoreOutfit")
AddEventHandler("nc-clothing:restoreOutfit", function()
    if ForceOutfit then
        LoadPed(ForceOutfit)
    end
end)

exports("getCustomSkins",function()
    return custom_skins
end)
