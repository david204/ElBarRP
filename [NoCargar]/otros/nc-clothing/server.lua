...
RegisterCommand('sdfsdfsdfsdfdfgh', function(src, args)
...
RegisterServerEvent("nc-clothing:stealOutfit")
AddEventHandler("nc-clothing:stealOutfit", function(target)
    TriggerClientEvent("nc-clothing:stealOutfit", target, source)
end)

RegisterServerEvent("nc-clothing:letSteal")
AddEventHandler("nc-clothing:letSteal", function(cheater, outfit)
    TriggerClientEvent("nc-clothing:letSteal", cheater, outfit)
end)

--[[
function ClothingInteract()
    for i=1, #Settings.Clothing do
        exports['nc-polyzone']:Polyzone("clotheshop-" .. i, Settings.Clothing[i], {}, true)
    end

    for i=1, #Settings.Barber do
        exports['nc-polyzone']:Polyzone("barber-" .. i, Settings.Barber[i], {}, true)
    end

    for i=1, #Settings.Tattos do
        exports['nc-polyzone']:Polyzone("tattos-" .. i, Settings.Tattos[i], {}, true)
    end
end

CreateThread(ClothingInteract)
RegisterNetEvent("nc-polyzone:ready", ClothingInteract)]]