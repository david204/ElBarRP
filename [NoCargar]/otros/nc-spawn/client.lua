...
RegisterNetEvent('nc-spawn:client:openUI', function(value)
...
RegisterNetEvent('nc-spawn:client:setupSpawns', function(cData, new, apps)
    if not new then
        QBCore.Functions.TriggerCallback('nc-spawn:server:getOwnedHouses', function(houses)
            local myHouses = {}
            if houses ~= nil then
                for i = 1, (#houses), 1 do
                    myHouses[#myHouses+1] = {
...
RegisterNUICallback('spawnplayer', function(data)
    ...
    if type == "current" then
        ...
        if insideMeta.house ~= nil then
            ...
            TriggerEvent('nc-houses:client:LastLocationHouse', houseId)
        elseif insideMeta.apartment.apartmentType ~= nil or insideMeta.apartment.apartmentId ~= nil then
            local apartmentType = insideMeta.apartment.apartmentType
            local apartmentId = insideMeta.apartment.apartmentId
            TriggerEvent('nc-apartments:client:LastLocationHouse', apartmentType, apartmentId)
        end
        TriggerServerEvent('QBCore:Server:OnPlayerLoaded')
        TriggerEvent('QBCore:Client:OnPlayerLoaded')
        PostSpawnPlayer()
    ...
    elseif type == "house" then
        PreSpawnPlayer()
        TriggerEvent('nc-houses:client:enterOwnedHouse', location)
        TriggerServerEvent('QBCore:Server:OnPlayerLoaded')
        TriggerEvent('QBCore:Client:OnPlayerLoaded')
        TriggerServerEvent('nc-houses:server:SetInsideMeta', 0, false)
        TriggerServerEvent('nc-apartments:server:SetInsideMeta', 0, 0, false)
        PostSpawnPlayer()
        TriggerServerEvent("clothing:checkIfNew")
    elseif type == "normal" then
