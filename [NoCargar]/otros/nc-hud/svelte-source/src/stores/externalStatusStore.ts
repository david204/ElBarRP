...
const store = () => {
  ...
  const methods = {
    receiveBuffStatusMessage(data: buffStatusMessage) {
      ...
      update(state => {
        ...
        if (!isNaN(data.progressValue) && data.progressValue >= 0) {
          ...
          console.error("nc-Buffs error: Buff State Message malformed!");
        }
    ...
    receiveEnhancementStatusMessage(data: enhancementStatusMessage) {
      ...
      if (!name) {
        console.error("nc-Buffs error: Enchancement Message name malformed:", data.enhancementName);
        return;
      }
      const playerIconName: iconNamesKind = name.replace('super-','') as iconNamesKind;

      if (!iconNames.includes(playerIconName)) {
        console.error("nc-Buffs error: Enhancement Message name not valid:", data.enhancementName);
        return;
      }
      ...
      update(state => {
        if (!state[playerIconName] && data.display && data.iconColor) {
          ...
          if (!state[playerIconName]) {
            console.error("nc-Buffs error: Enchancement name not found:", data.enhancementName);
            return state;
          }
          ...
          console.error("nc-Buffs error: Enhancement Message malformed:", data);
        }
