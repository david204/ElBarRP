...
local function hasHarness()
    ...
    local hasHarness = exports['nc-smallresources']:HasHarness()
    if hasHarness then
        _harness = true
    else
        _harness = false
...
RegisterNetEvent('hud:client:UpdateNeeds', function(newHunger, newThirst) -- Triggered in nchub
    hunger = newHunger
    thirst = newThirst
end)
...
RegisterNetEvent("nc-admin:client:ToggleDevmode", function()
    dev = not dev
end)
...
RegisterNetEvent('hud:client:BuffEffect', function(data)
    ...
    elseif data.display ~= nil then
        ...
        print("nc-Hud error: data invalid from client event call: hud:client:BuffEffect")
    end
end)
...
RegisterNetEvent('hud:client:EnhancementEffect', function(data)
    ...
    elseif data.display ~= nil then
        ...
        print("nc-Hud error: data invalid from client event call: hud:client:EnhancementEffect")
    end
end)
        ...
        if LocalPlayer.state.isLoggedIn then
            ...
            if IsPedInAnyVehicle(ped, false) then
                ...
                if speed >= stressSpeed then
                    ...
                    TriggerServerEvent('nc-level:up', 40)
                end
            end
        end
        Wait(10000)
