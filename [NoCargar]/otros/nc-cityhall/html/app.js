...
qbCityhall.SetupJobInfo = function (data) {
    var elem = '<div class="job-info-title"><span>'+ data.job.title +'</span></div>' +
        '<div class="job-info-box"><span id="info-label">Daily Salary: </span><span class="job-info-js">$'+ data.job.salary +'</span></div>' + 
...
$('.cityhall-option-block').click(function(e){
    ...
    if (blockPage == "identity") {
        ...
        $.post('https://nc-cityhall/requestLicenses', JSON.stringify({}), function(licenses){
...
$(".request-identity-button").click(function(e){
    ...
    $.post('https://nc-cityhall/requestId', JSON.stringify({
...
$(document).on("click", ".job-page-block", function(e){
    ...
    if (selectedJob == null) {
        ...
        selectedJob = this;
        $.post('https://nc-cityhall/setJobInfo', JSON.stringify({ job: selectedJobId }))
...
$(document).on('click', '.gps-job-button', function(e){
    ...
    $.post('https://nc-cityhall/setGps', JSON.stringify({
        job: selectedJobId
    }))
...
$(document).on('click', '.apply-job-button', function(e){
    ...
    $.post('https://nc-cityhall/applyJob', JSON.stringify({
        job: selectedJobId
    }))
