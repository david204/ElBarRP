...
QBCore.Functions.CreateUseableItem("tunerlaptop", function(source)
    TriggerClientEvent('nc-tunerchip:client:openChip', source)
end)

RegisterNetEvent('nc-tunerchip:server:TuneStatus', function(plate, bool)
    if bool then
...
QBCore.Functions.CreateCallback('nc-tunerchip:server:HasChip', function(source, cb)
    local src = source
...
QBCore.Functions.CreateCallback('nc-tunerchip:server:GetStatus', function(_, cb, plate)
