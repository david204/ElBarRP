...
RegisterNUICallback('save', function(data, cb)
    QBCore.Functions.TriggerCallback('nc-tunerchip:server:HasChip', function(HasChip)
        if HasChip then
            ...
            TriggerServerEvent('nc-tunerchip:server:TuneStatus', QBCore.Functions.GetPlate(veh), true)
        end
        cb('ok')
    end)
end)

RegisterNetEvent('nc-tunerchip:client:TuneStatus', function()
    local ped = PlayerPedId()
    ...
    if vehModel ~= 0 then
        QBCore.Functions.TriggerCallback('nc-tunerchip:server:GetStatus', function(status)
            if status then
...
RegisterNetEvent('nc-tunerchip:client:openChip', function()
    local ped = PlayerPedId()
