...
const OpenMenu = (data) => {
    ...
    $("#nc-input-form").on("change", function (event) {
        formInputs[$(event.target).attr("name")] = $(event.target).val();
    });

    $("#nc-input-form").on("submit", async function (event) {
        if (event != null) {
            event.preventDefault();
        }
        let formData = $("#nc-input-form").serializeArray();
