...
local function EnableEditMode()
	...
	CreateEditCamera()
	DecoMode = true
	TriggerEvent('nc-anticheat:client:ToggleDecorate', true)
end
...
local function SaveDecorations()
	if ClosestHouse then
		...
		TriggerServerEvent("nc-houses:server:savedecorations", ClosestHouse, ObjectList)
	end
end
...
local function DisableEditMode()
	...
	peanut = false
	DecoMode = false
	TriggerEvent('nc-anticheat:client:ToggleDecorate', false)
end
...
RegisterNetEvent('nc-houses:client:decorate', function()
	Wait(500)
	if IsInside then
		if HasKey then
...
RegisterNUICallback("buySelectedObject", function(data, cb)
    QBCore.Functions.TriggerCallback('nc-houses:server:buyFurniture', function(isSuccess)
        if isSuccess then
            SetNuiFocus(false, false)
            cursorEnabled = false
            SaveDecorations()
