...
CreateThread(function()
    ...
    TriggerClientEvent("nc-garages:client:houseGarageConfig", -1, HouseGarages)
    TriggerClientEvent("nc-houses:client:setHouseConfig", -1, Config.Houses)
end)
...
QBCore.Commands.Add("decorate", "Decorate Interior", {}, false, function(source)
    local src = source
    TriggerClientEvent("nc-houses:client:decorate", src)
end)
...
QBCore.Commands.Add("createhouse", "Create House (Real Estate Only)", {{name = "price", help = "Price of the house"}, {name = "tier", help = "Name of the item(no label)"}}, true, function(source, args)
    ...
    if Player.PlayerData.job.name == "realestate" then
        TriggerClientEvent("nc-houses:client:createHouses", src, price, tier)
    else
...
QBCore.Commands.Add("addgarage", "Add House Garage (Real Estate Only)", {}, false, function(source)
    ...
    local Player = QBCore.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "realestate" then
        TriggerClientEvent("nc-houses:client:addGarage", src)
    else
        TriggerClientEvent('QBCore:Notify', src, "Only realestate can use this command", "error")
    end
end)
...
QBCore.Commands.Add("enter", "Enter House", {}, false, function(source)
    local src = source
    TriggerClientEvent('nc-houses:client:EnterHouse', src)
end)

QBCore.Commands.Add("ring", "Ring The Doorbell", {}, false, function(source)
    local src = source
    TriggerClientEvent('nc-houses:client:RequestRing', src)
end)
...
QBCore.Functions.CreateUseableItem("police_stormram", function(source, item)
    ...
    if (Player.PlayerData.job.name == "police" and Player.PlayerData.job.onduty) then
        TriggerClientEvent("nc-houses:client:HomeInvasion", source)
    else
...
RegisterNetEvent('nc-houses:server:setHouses', function()
    local src = source
    TriggerClientEvent("nc-houses:client:setHouseConfig", src, Config.Houses)
end)

RegisterNetEvent('nc-houses:server:createBlip', function()
    local src = source
    local ped = GetPlayerPed(src)
    local coords = GetEntityCoords(ped)
    TriggerClientEvent("nc-houses:client:createBlip", -1, coords)
end)

RegisterNetEvent('nc-houses:server:addNewHouse', function(street, coords, price, tier)
    local src = source
    ...
    TriggerEvent('nc-log:server:CreateLog', 'house', 'House Created:', 'green', '**Address**:\n'..label..'\n\n**Listing Price**:\n$'..price..'\n\n**Tier**:\n'..tier..'\n\n**Listing Agent**:\n'..GetPlayerName(src))
end)

RegisterNetEvent('nc-houses:server:addGarage', function(house, coords)
    local src = source
    ...
    TriggerClientEvent("nc-garages:client:addHouseGarage", -1, house, garageInfo)
    TriggerClientEvent('QBCore:Notify', src, "You have added a garage: " .. garageInfo.label)
end)

RegisterNetEvent('nc-houses:server:viewHouse', function(house)
    local src = source
    ...
    TriggerClientEvent('nc-houses:client:viewHouse', src, houseprice, brokerfee, bankfee, taxes,
        pData.PlayerData.charinfo.firstname, pData.PlayerData.charinfo.lastname)
end)

RegisterNetEvent('nc-houses:server:buyHouse', function(house)
    local src = source
    local pData = QBCore.Functions.GetPlayer(src)
    ...
    if (bankBalance >= HousePrice) then
        ...
        TriggerClientEvent('nc-houses:client:SetClosestHouse', src)
        pData.Functions.RemoveMoney('bank', HousePrice, "bought-house") -- 21% Extra house costs
        TriggerEvent('nc-bossmenu:server:addAccountMoney', "realestate", (HousePrice / 100) * math.random(18, 25))
        TriggerEvent('nc-log:server:CreateLog', 'house', 'House Purchased:', 'green', '**Address**:\n'..house:upper()..'\n\n**Purchase Price**:\n$'..HousePrice..'\n\n**Purchaser**:\n'..pData.PlayerData.charinfo.firstname..' '..pData.PlayerData.charinfo.lastname)
    else
        TriggerClientEvent('QBCore:Notify', source, "You dont have enough money..", "error")
    end
end)

RegisterNetEvent('nc-houses:server:lockHouse', function(bool, house)
    TriggerClientEvent('nc-houses:client:lockHouse', -1, bool, house)
end)

RegisterNetEvent('nc-houses:server:SetRamState', function(bool, house)
    Config.Houses[house].IsRaming = bool
    TriggerClientEvent('nc-houses:server:SetRamState', -1, bool, house)
end)

RegisterNetEvent('nc-houses:server:giveKey', function(house, target)
...
RegisterNetEvent('nc-houses:server:removeHouseKey', function(house, citizenData)
    local src = source
    local newHolders = {}
    if housekeyholders[house] then
...
RegisterNetEvent('nc-houses:server:OpenDoor', function(target, house)
    local OtherPlayer = QBCore.Functions.GetPlayer(target)
    if OtherPlayer then
        TriggerClientEvent('nc-houses:client:SpawnInApartment', OtherPlayer.PlayerData.source, house)
    end
end)

RegisterNetEvent('nc-houses:server:RingDoor', function(house)
    local src = source
    TriggerClientEvent('nc-houses:client:RingDoor', -1, src, house)
end)

RegisterNetEvent('nc-houses:server:savedecorations', function(house, decorations)
    exports.oxmysql:execute('UPDATE player_houses SET decorations = ? WHERE house = ?', {json.encode(decorations), house})
    TriggerClientEvent("nc-houses:server:sethousedecorations", -1, house, decorations)
end)

RegisterNetEvent('nc-houses:server:LogoutLocation', function()
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent('nc-houses:server:giveHouseKey', function(target, house)
    local src = source
    ...
    if tPlayer then
        if housekeyholders[house] then
            ...
            exports.oxmysql:execute('UPDATE player_houses SET keyholders = ? WHERE house = ?', {json.encode(housekeyholders[house]), house})
            TriggerClientEvent('nc-houses:client:refreshHouse', tPlayer.PlayerData.source)
...
RegisterNetEvent('nc-houses:server:setLocation', function(coords, house, type)
    if type == 1 then
    ...
    TriggerClientEvent('nc-houses:client:refreshLocations', -1, house, json.encode(coords), type)
end)

RegisterNetEvent('nc-houses:server:SetHouseRammed', function(bool, house)
    Config.Houses[house].IsRammed = bool
    TriggerClientEvent('nc-houses:client:SetHouseRammed', -1, bool, house)
end)

RegisterNetEvent('nc-houses:server:SetInsideMeta', function(insideId, bool)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
QBCore.Functions.CreateCallback('nc-houses:server:buyFurniture', function(source, cb, price)
    local src = source
    local pData = QBCore.Functions.GetPlayer(src)
    local bankBalance = pData.PlayerData.money["bank"]
...
QBCore.Functions.CreateCallback('nc-houses:server:ProximityKO', function(source, cb, house)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    local retvalK = false
    local retvalO = false
    ...
    if Player then
        ...
        if hasKey(identifier, CharId, house) then
            retvalK = true
        elseif Player.PlayerData.job.name == "none" then
            retvalK = true
        else
            retvalK = false
        end
...
QBCore.Functions.CreateCallback('nc-houses:server:hasKey', function(source, cb, house)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    local retval = false
    if Player then
...
QBCore.Functions.CreateCallback('nc-houses:server:isOwned', function(source, cb, house)
    if houseowneridentifier[house] and houseownercid[house] then
        cb(true)
    else
        cb(false)
...
QBCore.Functions.CreateCallback('nc-houses:server:getHouseOwner', function(source, cb, house)
    cb(houseownercid[house])
end)

QBCore.Functions.CreateCallback('nc-houses:server:getHouseKeyHolders', function(source, cb, house)
    local retval = {}
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if housekeyholders[house] then
...
QBCore.Functions.CreateCallback('nc-houses:server:getHouseDecorations', function(source, cb, house)
    local retval = nil
...
QBCore.Functions.CreateCallback('nc-houses:server:getHouseLocations', function(source, cb, house)
    local retval = nil
    local result = exports.oxmysql:executeSync('SELECT * FROM player_houses WHERE house = ?', {house})
    if result[1] then
        retval = result[1]
...
QBCore.Functions.CreateCallback('nc-houses:server:getHouseKeys', function(source, cb)
    local src = source
    local pData = QBCore.Functions.GetPlayer(src)
    local cid = pData.PlayerData.citizenid
end)

QBCore.Functions.CreateCallback('nc-houses:server:getOwnedHouses', function(source, cb)
    local src = source
    local pData = QBCore.Functions.GetPlayer(src)
    if pData then
...
QBCore.Functions.CreateCallback('nc-houses:server:getSavedOutfits', function(source, cb)
    local src = source
    local pData = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent('nc-houses:server:updateTier', function()
    local HouseGarages = {}
    local result = exports.oxmysql:executeSync('SELECT * FROM houselocations', {})
...
RegisterNetEvent('nc-houses:server:updatePrice', function(price)
...
RegisterNetEvent('nc-houses:server:deletehouses', function(selectedHouse)
