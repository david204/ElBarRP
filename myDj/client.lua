...
Citizen.CreateThread(function()
    while true do
        ...
        for k, v in pairs(Config.DJPositions) do
            if (PlayerJob and v.requiredJob == PlayerJob.name) then
...
Citizen.CreateThread(function()
    while true do
        ...
        if currentDJ then
            if xSound and xSound:soundExists(currentDJ.name) then
...
RegisterNUICallback('close', function(data, herecb) 
...
RegisterNUICallback('togglePlaystate', function(data,herecb)
...
RegisterNUICallback('playNewSong', function(data,herecb) 
...
RegisterNUICallback('playSongFromPlaylist', function(data,herecb)
    -- Song ID = data.id
...
RegisterNUICallback('rewind', function(data,herecb)
    -- 10 secs zurück 
...
RegisterNUICallback('forward', function(data,herecb)
    -- 10 secs vorwärts 
...
RegisterNUICallback('down', function(data,herecb)
    -- leiser machen
...
RegisterNUICallback('up', function(data,herecb)
    -- lauter machen
...
RegisterNUICallback('addPlayList', function(data,herecb)
    -- Playlist hinzufügen
    -- label = data.name
    TriggerServerEvent('myDJ:addPlaylist', data.name)
end)

RegisterNUICallback('addSongToPlaylist', function(data,herecb)
    -- link = data.link
    -- Playlist ID = data.id
...
RegisterNUICallback('deleteSong', function(data,herecb)
    -- Song ID = data.id
    -- Playlist ID = data.playlistId
...
RegisterNUICallback('deletePlaylist', function(data,herecb)
    -- Song ID = data.id
    -- Playlist ID = data.playlistId
...
RegisterNUICallback('noSongtitle', function(data,herecb)
