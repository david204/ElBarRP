    ...
    TriggerCallback('fleeca:server:checkPoliceCount', function(data)
        if data['status'] then
            TriggerCallback('fleeca:server:checkTime', function(data)
                if data['status'] then
                    SetupFleeca(index)
                    TriggerServerEvent('fleeca:server:policeAlert', Config['FleecaSetup'][index]['main'])
                end
            end, index)
        end
    end)
...
function SetupFleeca(index)
    loadModel('test_trolly')
    loadModel('h4_prop_h4_cash_stack_01a')
    trolly = CreateObject(GetHashKey('test_trolly'), Config['FleecaSetup'][index]['trolly']['coords'], 1, 1, 0)
...
function DrillDoor(a, b)
    ...
    TriggerCallback('fleeca:server:hasItem', function(data)
        if data['status'] then
            local HasItem = isHasItem("security_card_03")
            if HasItem then
            exports['nc-dispatch']:FleecaBankRobbery()
            TriggerServerEvent('fleeca:server:sync', 'action', {'door'.. b, a})
            ...
            QBCore.Functions.Notify('You Missed A Card', 'error')
        end
        else
            QBCore.Functions.Notify('You Missed A Big Drill', 'error')
        end
        ...
        while GetGameTimer() - startedGrabbing < 37000 do
            ...
            if HasAnimEventFired(ped, GetHashKey("RELEASE_CASH_DESTROY")) then
                if IsEntityVisible(grabobj) then
                    SetEntityVisible(grabobj, false, false)
                    TriggerServerEvent('fleeca:server:rewardItem', Config['FleecaHeist']['rewardItems'][3]['itemName'], Config['FleecaHeist']['rewardItems'][3]['count'], 'item', index)
                end
            end
        end
        ...
        while true do
            ...
            if dist <= 15.0 then
                ...
                if math.random (1, 100) > 50 then 
                    TriggerServerEvent("nc-fleeca:yonatan:GiveCard")
                end
                if math.random(1, 100) > 10 then
                    TriggerServerEvent("nc-fleeca:yonatan:GreenUSB")
                end
                break
            end
            Wait(1)
