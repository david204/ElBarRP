...
RegisterNetEvent('nc-blackout:client:startblackout', function ()
	QBCore.Functions.TriggerCallback("nc-blackout:server:getCops", function(enoughCops)
    if enoughCops >= Config.MinimumPolice then
        QBCore.Functions.TriggerCallback("nc-blackout:server:coolc",function(isCooldown)
            if not isCooldown then
                QBCore.Functions.Progressbar("search_register", "Preparing Explosive", 3000, false, true, {
                    ...
                    bombanime()
					TriggerServerEvent('nc-blackout:server:startr')
                    blackout = true
                end, function() -- Cancel
...
RegisterNetEvent('nc-bombplant')
AddEventHandler('nc-bombplant', function()
    QBCore.Functions.TriggerCallback('QBCore:HasItem', function(hasItem)
        if hasItem then
            exports["memorygame"]:thermiteminigame(4, 3, 5, 10,
            function() -- success
            TriggerEvent('nc-blackout:client:startblackout')			
            Citizen.Wait(1000)
            TriggerEvent("nc-blackout")
        end,
        function() -- failure
        end)
        else
...
RegisterNetEvent('nc-blackout')
AddEventHandler('nc-blackout', function()
		Citizen.Wait(5500)
...
RegisterNetEvent('nc-blackoutfix')
AddEventHandler('nc-blackoutfix', function()
	TriggerEvent("chat:addMessage", {
        color = {255, 255, 255},
        -- multiline = true,
        template = '<div style="padding: 15px; margin: 15px; background-color: rgba(180, 117, 22, 0.9); border-radius: 15px;"><i class="far fa-building"style="font-size:15px"></i> | {0} </font></i></b></div>',
