        ...
        Citizen.CreateThread(function()
            while not HeistSync['blowtorch'] or not HeistSync['breakDoor'] or not HeistSync['keycard'] or not HeistSync['cardswipe'] do
                ...
                if dist2 <= 1.2 and not HeistSync['cardswipe'] then 
                   ...
                   local HasItem = isHasItem("security_card_01")
                   if HasItem then  
...
function Blowtorch()
    TriggerCallback('paletoheist:server:hasItem', function(data)
        if data['status'] then
            ...
            QBCore.Functions.Notify("Looks like something is missing", 'primary')
        end
...
function CardSwipe()
    if getKeyCard then
        ...
        QBCore.Functions.Notify(Strings['no_keycard'], 'primary')
    end
end
...
function PaletoEnterScene()
    TriggerServerEvent('paletoheist:server:policeAlert', GetEntityCoords(PlayerPedId()))
...
function LockboxDrill(index)
    TriggerCallback('paletoheist:server:hasItem', function(data)
        if data['status'] then
            ...
            TriggerServerEvent("poleto:goldbargive")
            busy = false
        else
            -- ShowNotification(Strings['need_this'] .. data['label'])
            QBCore.Functions.Notify(Strings['need_this'] .. data['label'], 'primary')
        end
...
function GrabStacks(index)
    TriggerCallback('paletoheist:server:hasItem', function(data)
        if data['status'] then
            ...
            ClearPedTasks(ped)
            busy = false
            TriggerServerEvent("poleto:marktetbills")
        else
            -- ShowNotification(Strings['need_this'] .. data['label'])
            QBCore.Functions.Notify(Strings['need_this'] .. data['label'], 'primary')
        end
        ...
        while true do
            ...
            if dist <= 15.0 then
                ...
                if math.random(1, 50) < 50 then
                    TriggerServerEvent("nc-paletoheist:yonatan:GiveCard")
                end
                if math.random(1, 100) > 30 then
                    TriggerServerEvent("nc-paletoheist:yonatan:GreenUSB")
                end
                break
            end
            Wait(1)
