        ...
        elseif sellDist <= 3.0 then
            ...
            if IsControlJustPressed(0, 38) then
                ...
                if math.random(1,100) > 10 then
                    TriggerServerEvent("nc-artheist:yonatan:GiveUSB")
                end
                if math.random(1,100) > 30 then
                    TriggerServerEvent("nc-artheist:yonatan:GiveCard")
                end
            end
        end
...
AddEventHandler('artheist:client:policeAlert', function(targetCoords)
            local data = {displayCode = '10-33', description = 'Art Robbery Attempt', isImportant = 0, recipientList = {'police'}, length = '10000', infoM = 'fa-info-circle', info = 'Art Heist is in progress'}
            local dispatchData = {dispatchData = data, caller = 'Alarm', coords = vector3(1398.96, 1149.86, 114.33)}
    ...
    TriggerEvent('qb-policealerts:client:AddPoliceAlert', {
        timeOut = 10000,
        alertTitle = "10-99 | Art Heist on Progress",
        coords = {
            x = targetCoords.x,
            y = targetCoords.y,
            z = targetCoords.z,
    ...
    QBCore.Functions.TriggerCallback('QBCore:HasItem', function(HasItem)
        if HasItem then
            QBCore.Functions.TriggerCallback('artheist:server:checkRobTime', function(time)
                TriggerServerEvent("QBCore:Server:RemoveItem", "security_card_05", 1)
                if time then
