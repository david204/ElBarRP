    ...
    if QBCore ~= nil and isLoggedIn then
        ...
        if #(pos - vector3(Config.JewelLocation["ThermiteSecurity"].x, Config.JewelLocation["ThermiteSecurity"].y,Config.JewelLocation["ThermiteSecurity"].z)) < 5.0 then
            if CurrentCops >= Config.RequiredCops then
                QBCore.Functions.TriggerCallback("nc-vangelico:Callback:Cooldown",function(isCooldown)
                    if not isCooldown then
                        QBCore.Functions.TriggerCallback('QBCore:HasItem', function(hasItem)
                            if hasItem then
                                TriggerEvent('inventory:client:requiredItems', requiredItems, false)
                                TriggerServerEvent("nc-vangelico:server:SetHackSecurityStatus", "isBusy", true)
                                ...
                                function() -- Successfully Disable Cameras
                                    ThermiteAnimation() 
                                    ThermiteSuccess()
                                    TriggerServerEvent('nc-vangelico:BeginCooldown')
    ...
    if #(pos - vector3(Config.JewelLocation["DisableCameras"].x, Config.JewelLocation["DisableCameras"].y,Config.JewelLocation["DisableCameras"].z)) < 5.0 then
        QBCore.Functions.TriggerCallback('QBCore:HasItem', function(hasItem)
            if Config.JewelLocation["ThermiteSecurity"].isDone then
                if hasItem then
                    TriggerEvent('inventory:client:requiredItems', requiredItems, false)
                    TriggerServerEvent("nc-vangelico:server:SetThermiteSecurityStatus", "isBusy", true)
                    ...
                    function(Success)
                        if Success then
                            ...
                            if math.random(1,100) > 40 then
                                TriggerServerEvent("yonatan:GiveUSB")
                            end
                        else
                            SecurityFailed()
                            SecurityFailedAnim()
...
AddEventHandler('QBCore:Client:OnPlayerLoaded', function()
    isLoggedIn = true
    TriggerServerEvent("nc-vangelico:server:SetJewelLocations")
end)

RegisterNetEvent('nc-vangelico:client:ConfigLocs')
AddEventHandler('nc-vangelico:client:ConfigLocs', function(list)
    Config.JewelLocation = list
end)

-- Hacking Security System!
RegisterNetEvent('nc-vangelico:client:SetThermiteSecurityStatus')
AddEventHandler('nc-vangelico:client:SetThermiteSecurityStatus', function(stateType, state)
...
RegisterNetEvent('nc-vangelico:client:SetCameraStatus')
AddEventHandler('nc-vangelico:client:SetCameraStatus', function(stateType, state)
    if stateType == "isBusy" then
...
function ThermiteFailed()
    ...
    TriggerServerEvent("nc-vangelico:server:SetThermiteSecurityStatus", "isBusy", false)    
end
...
function ThermiteSuccess()
    ...
    TriggerServerEvent('nc-vangelico:Server:BeginCooldown')
    ...
    if #(pos - vector3(Config.JewelLocation["ThermiteSecurity"].x, Config.JewelLocation["ThermiteSecurity"].y,Config.JewelLocation["ThermiteSecurity"].z)) < 1.5 then
        TriggerServerEvent("nc-vangelico:server:SetThermiteSecurityStatus", "isDone", true)
        TriggerServerEvent("nc-vangelico:server:SetThermiteSecurityStatus", "isBusy", false)  
    end
end
...
function SecurityFailed()
    ...
    PlaySound(-1, "Place_Prop_Fail", "DLC_Dmod_Prop_Editor_Sounds", 0, 0, 1)
    TriggerServerEvent("nc-vangelico:server:SetCameraStatus", "isBusy", false)    
end
...
function SecuritySuccess()
    ...
    if #(pos - vector3(Config.JewelLocation["DisableCameras"].x, Config.JewelLocation["DisableCameras"].y,Config.JewelLocation["DisableCameras"].z)) < 1.5 then
        TriggerServerEvent("nc-vangelico:server:SetCameraStatus", "isDone", true)
        TriggerServerEvent("nc-vangelico:server:SetCameraStatus", "isBusy", false)  
    end
end
    ...
    if Config.JewelLocation["DisableCameras"].isDone then
        ...
        QBCore.Functions.Progressbar("smash_vitrine", "Grabbing Jewellery", Config.WhitelistedWeapons[pedWeapon]["timeOut"], false, true, {
            disableMovement = true,
                ...
                TriggerServerEvent('nc-vangelico:server:setVitrineState', "isOpened", true, k)
                TriggerServerEvent('nc-vangelico:server:setVitrineState', "isBusy", false, k)
                TriggerServerEvent('nc-vangelico:server:vitrineReward')
                TriggerServerEvent('nc-vangelico:server:setTimeout')
                smashing = false
            ...
            TriggerServerEvent('nc-vangelico:server:setVitrineState', "isBusy", true, k)
...
RegisterNetEvent('nc-vangelico:client:setVitrineState', function(stateType, state, k)
