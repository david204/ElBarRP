...
function StartHeist()
    QBCore.Functions.TriggerCallback('pacificheist:server:checkPoliceCount', function(status)
        if status then
            local HasItem = isHasItem("security_card_02")
            if HasItem then
                ...
                if time then
                    QBCore.Functions.Notify(Strings['start_heist'], "success")
                    QBCore.Functions.Notify(Strings['start_heist2'], "success")
            ...
            QBCore.Functions.Notify("There aren't enough cops", 'error')
        end
    end)
end
...
function PlantingAnim(type, index, pos, rot)
    ...
    QBCore.Functions.TriggerCallback('pacificheist:server:hasItem', function(hasItem, itemLabel)
        if hasItem then
            exports['nc-dispatch']:PaletoBankRobbery()
            busy = true
        ...
        while true do
            ...
            if dist <= 15.0 then
                ...
                if math.random(1,100) > 90 then
                    TriggerServerEvent("pacificheist:yonatan:VARHelmet")
                end
                if math.random(1,100) > 2 then
                    TriggerServerEvent("pacificheist:yonatan:BlueUSB")
                end
                if math.random(1,100) > 10 then
                    TriggerServerEvent("pacificheist:yonatan:GreenfUSB")
                end
                if math.random(1,100) > 15 then
                    TriggerServerEvent("pacificheist:yonatan:GoldUSB")
                end
                break
            end
            Wait(1)
