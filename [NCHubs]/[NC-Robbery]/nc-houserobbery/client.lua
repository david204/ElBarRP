...
AddEventHandler('lockpicks:UseLockpick', function(advanced)
 ...
 if isNight() and not isRobbing then
  QBCore.Functions.TriggerCallback("srp:getCopsCount", function(copsNum)
    if copsNum >= Config.MinCops then
      local playerCoords = GetEntityCoords(PlayerPedId(), true)
      for id,v in pairs(Config.RobbableHouses) do
        if GetDistanceBetweenCoords(playerCoords, v.x, v.y, v.z, true) <= 2.5 then

          local success = exports['nc-lock']:StartLockPickCircle(10,6) --StartLockPickCircle(1,10) 1= how many times, 30 = time in seconds
          if success then
            QBCore.Functions.Progressbar("lockpicking_property", "Lockpicking Property", 12000, false, true, {}, {}, {}, {}, function()
              ...
              TriggerServerEvent('houseRobberies:log', id, true);
            end) 
          else
            QBCore.Functions.Notify("Lockpicking failed!", "error")
            TriggerEvent('dispatch:houseRobbery')
            TriggerServerEvent('houseRobberies:log', id, false);
          end
        end
      end
    else
...
if not isNight() and not isRobbing then
  TriggerEvent('QBCore:Notify', 'There are not enough cops.', "error")
end
end)
   ...
   for i=1,#myRobbableItems do
    if (GetDistanceBetweenCoords(generator.x + myRobbableItems[i]["x"], generator.y + myRobbableItems[i]["y"], generator.z + myRobbableItems[i]["z"], GetEntityCoords(GetPlayerPed(-1))) < 1.4) and not myRobbableItems[i]['isSearched'] then
     ...
     if IsControlJustReleased(1, 74) then
      ...
      local success = exports['nc-lock']:StartLockPickCircle(6,6) --StartLockPickCircle(1,10) 1= how many times, 30 = time in seconds
      if success then 
...
function buildBasicHouse(generator)
 ...
 if false and math.random(1, 10) >= 6 then
