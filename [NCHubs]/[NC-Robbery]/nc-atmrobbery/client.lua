...
RegisterNetEvent('nc-atmrobbery:client:roubar', function()
    RobAtm()
end)
...
function RobAtm()
	...
	if LocalPlayer.state.isLoggedIn then
		QBCore.Functions.TriggerCallback("nc-atmrobbery:Cooldown", function(cooldown)
...
function RobbingTheMoney()
    ...
    Citizen.CreateThread(function()
        while Anim do
            TriggerServerEvent('nc-hud:Server:gain:stress', math.random(2, 3))
            Citizen.Wait(12000)
        end
    end)
end
...
function HackSuccess()
    ...
    ClearPedTasksImmediately(PlayerPedId())
	RobbingTheMoney()
	TriggerServerEvent("nc-atmrobbery:success")	
    TriggerServerEvent('nc-atmrobbery:Server:BeginCooldown')
end
...
local prop = {
  ...
  exports['qb-target']:AddTargetModel(prop, {
      options = {
          {
              type = "client",
              event = "nc-atmrobbery:client:roubar",
