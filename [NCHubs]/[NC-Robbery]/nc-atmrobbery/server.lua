...
-- Recompensa
RegisterServerEvent("nc-atmrobbery:success")
AddEventHandler("nc-atmrobbery:success",function()
    local src = source
	local Player =  QBCore.Functions.GetPlayer(source)
    local bags = 1
	local info = {
...
-- Cooldown
RegisterServerEvent('nc-atmrobbery:Server:BeginCooldown')
AddEventHandler('nc-atmrobbery:Server:BeginCooldown', function()
    Cooldown = true
    local timer = 60 * 1000
    while timer > 0 do
        Wait(1000)
...
QBCore.Functions.CreateCallback("nc-atmrobbery:Cooldown",function(source, cb)
    if Cooldown then
        cb(true)
    else
        cb(false)
