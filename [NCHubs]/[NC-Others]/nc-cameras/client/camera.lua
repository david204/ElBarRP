...
AddEventHandler('QBCore:Client:OnPlayerLoaded', function()
    TriggerServerEvent("nc-cameras:server:RequestSync")
end)

RegisterNetEvent('nc-cameras:client:SyncData')
AddEventHandler('nc-cameras:client:SyncData', function(data)
    Config.SecurityCameras.cameras = data
end)

-- 
RegisterNetEvent('nc-cameras:client:DisableCam')
AddEventHandler('nc-cameras:client:DisableCam', function(camID, enable)
...
RegisterNetEvent('nc-cameras:client:RemoveCamera')
AddEventHandler('nc-cameras:client:RemoveCamera', function(camID, isOwner, entity)
    if isOwner then
        for k, v in pairs(Config.SecurityCameras.cameras) do 
            if v.camID == camID then 
...
RegisterNetEvent('nc-cameras:client:OpenMenu')
AddEventHandler('nc-cameras:client:OpenMenu', function()
    if not opened then 
        SendNUIMessage({
            type = "ui",
            status = true, 
...
RegisterNUICallback('OpenCamera', function(data)
    TriggerEvent("nc-cameras:client:ActiveCamera", tonumber(data.camid))
end)

RegisterNetEvent('nc-cameras:client:UseCCTVCamera')
AddEventHandler('nc-cameras:client:UseCCTVCamera', function(is360, GcamID)
                ...
                if hasItem then 
                    QBCore.Functions.Progressbar("fixing camera", "Placing CCTV Camera", 2000, false, true, {
                        ...
                        if is360 == 1 then 
                            ...
                            TriggerServerEvent("nc-cameras:server:AddCamera", Config.SecurityCameras.cameras[id], true, cam)
                            ...
                            TriggerServerEvent("nc-cameras:server:AddCamera", Config.SecurityCameras.cameras[id], false, cam)
                        end
                        exports['qb-target']:AddEntityZone("camera-"..GcamID, cam, {
                            ...
                            options = {
                                {
                                    type = "server",
                                    event = "nc-cameras:server:DisableCam",
                                    icon = "fa fa-camera",
                                ...
                                {
                                    type = "server",
                                    event = "nc-cameras:server:RemoveCamera",
                                    icon = "fa fa-camera",
...
RegisterNetEvent('nc-cameras:client:ActiveCamera')
AddEventHandler('nc-cameras:client:ActiveCamera', function(cameraId)
...
RegisterNetEvent("nc-cameras:client:CreateCamerasAndTarget")
AddEventHandler("nc-cameras:client:CreateCamerasAndTarget", function(data, player)
