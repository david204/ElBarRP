...
QBCore.Functions.CreateUseableItem("cctv", function(source, item)
    ...
    if Player.Functions.GetItemBySlot(item.slot) ~= nil then
        TriggerClientEvent("nc-cameras:client:UseCCTVCamera", source, 0, GcamID)
    end
end)
...
QBCore.Functions.CreateUseableItem("360cctv", function(source, item)
    local Player = QBCore.Functions.GetPlayer(source)
    if Player.Functions.GetItemBySlot(item.slot) ~= nil then
        TriggerClientEvent("nc-cameras:client:UseCCTVCamera", source, 1, GcamID)
    end
end)
...
QBCore.Functions.CreateUseableItem("camviewer", function(source, item)
    local Player = QBCore.Functions.GetPlayer(source)
    if Player.Functions.GetItemBySlot(item.slot) ~= nil then
        TriggerClientEvent("nc-cameras:client:OpenMenu", source)
    end
end)
...
Citizen.CreateThread(function()
    exports.oxmysql:query('SELECT * FROM nccameras', {}, function(result)
...
RegisterServerEvent("nc-cameras:server:RequestSync")
AddEventHandler("nc-cameras:server:RequestSync", function()
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    local cid = Player.PlayerData.citizenid

    if SecurityCameras[cid] ~= nil then 
        TriggerClientEvent("nc-cameras:client:SyncData", src, SecurityCameras[cid])
        TriggerClientEvent("nc-cameras:client:CreateCamerasAndTarget", src, FetchedData, 0)
    end

    -- Citizen.Wait(5000)
    -- for k, v in pairs(NetID) do 
    --     TriggerClientEvent("nc-cameras:client:NewCameraTarget", src, k)
    -- end
end)

RegisterServerEvent("nc-cameras:server:DisableCam")
AddEventHandler("nc-cameras:server:DisableCam", function(data)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    ...
    if FetchedData[numid] ~= nil then 
        ...
        if dt ~= nil then 
            if dt.isOnline ~= data.enable then
                ...
                if ply ~= nil then
                    TriggerClientEvent("nc-cameras:client:DisableCam", ply.PlayerData.source, numid, data.enable)
                end
                if data.enable then 
...
RegisterServerEvent("nc-cameras:server:RemoveCamera")
AddEventHandler("nc-cameras:server:RemoveCamera", function(data)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    local numid = tonumber(data.camera)
    ...
    if FetchedData[numid] ~= nil then 
        ...
        local ply = QBCore.Functions.GetPlayerByCitizenId(cid)
        if ply ~= nil then
            TriggerClientEvent("nc-cameras:client:RemoveCamera", ply.PlayerData.source, numid, true, tonumber(data.ent))
        end
        if is360 then
        ...
        TriggerClientEvent("nc-cameras:client:RemoveCamera", src, numid, false, tonumber(data.ent))
        exports.oxmysql:query('DELETE FROM `nccameras` WHERE `camID` = ?', {numid})
        TriggerClientEvent("QBCore:Notify", src, "Camera was disconnected and put into your inventory.", "success")
    end
end)

RegisterServerEvent("nc-cameras:server:AddCamera")
AddEventHandler("nc-cameras:server:AddCamera", function(data, is360, cam)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    local cid = Player.PlayerData.citizenid
    ...
    -- TriggerClientEvent("nc-cameras:client:NewCameraTarget", -1, netid)
    ...
    exports.oxmysql:insert('INSERT INTO nccameras (cid, camID, cameradata) VALUES (?, ?, ?) ', {cid, GcamID, json.encode(SecurityCameras[cid][index])}, function(id)
        GcamID = GcamID + 1
    end)

    local mainTable = {SecurityCameras[cid][index]}
    TriggerClientEvent("nc-cameras:client:CreateCamerasAndTarget", -1, mainTable, src)
