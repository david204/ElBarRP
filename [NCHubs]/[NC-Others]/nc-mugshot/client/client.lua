...
RegisterNetEvent('nc-mugshot:client:trigger', function()
    ped = PlayerPedId()
    ...
    CreateThread(function()
        ...
        if Config.CQCMDT then
            TriggerServerEvent('nc-mugshot:server:MDTupload', playerData.citizenid, MugshotArray)
        end
...
RegisterNetEvent("nc-mugshot:client:takemugshot",function()
    local player, distance = QBCore.Functions.GetClosestPlayer(GetEntityCoords(PlayerPedId()))
    if player ~= -1 and distance < 2.0 then
        local playerId = GetPlayerServerId(player)
        TriggerServerEvent('nc-mugshot:server:triggerSuspect', playerId)
    end
end)
...
if Config.TestCommand then
    ...
    RegisterCommand("testmugshotself", function(source)
        TriggerEvent('nc-mugshot:client:trigger')
    end, false)
end