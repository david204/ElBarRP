...
function StopTheoryTest(success)
    ...
    SetNuiFocus(false)
    if success then
        QBCore.Functions.Notify("Aprobaste el examen teórico, ¡comienza el examen práctico!", "Conseguido", 5000)
        TriggerServerEvent('qb-drivingschool:server:givepermit')
    else
        QBCore.Functions.Notify("Teorico suspendido", "error")
    end
end
...
function StopDriveTest(success)
    if success then
        QBCore.Functions.Notify("Aprobaste el examen teórico", "Conseguido")
...
AddEventHandler('qb-drivingschool:hasEnteredMarker', function(zone)
    if zone == 'DMVSchool' then
        ...
        CurrentActionMsg = ('Aprieta la E para pagar el acceso al examen - $500')
        CurrentActionData = {}
    end
end)
...
CreateThread(function()
    ...
    AddTextComponentString('Autoescuela')
    ...
    while true do
        ...
        if CurrentTest == 'drive' then
            ...
            if Config.CheckPoints[nextCheckPoint] == nil then
                ...
                CurrentTest = nil
                QBCore.Functions.Notify("Examen de conduccion completo", "error")
        ...
        if CurrentTest == 'drive' then
            ...
            if IsPedInAnyVehicle(playerPed, false) then
                ...
                for k, v in pairs(Config.SpeedLimits) do
                    if CurrentZoneType == k and speed > v then
                        ...
                        if not IsAboveSpeedLimit then
                            ...
                            QBCore.Functions.Notify("¡Conduces demasiado rápido! Velocidad permitida: " .. v .. " ", "error")
                            QBCore.Functions.Notify("Errores - " .. DriveErrors .. " / " .. Config.MaxErrors .. " ", "error")
                        end
                    end
                end
                ...
                if health < LastVehicleHealth then
                    DriveErrors = DriveErrors + 1
                    QBCore.Functions.Notify("ESTAS LOCO! ¿NOS QUIERES MATAR? ¡ESE GOLPE LO PAGAS TU!", "error")
                    QBCore.Functions.Notify("Errores - " .. DriveErrors .. " / " .. Config.MaxErrors .. " ", "error")
...
RegisterNetEvent('opendriving', function()
        ...
        QBCore.Functions.TriggerCallback('qb-drivingschool:server:HasLicense', function(HasItem)
                ...
                if HasItem then
                    QBCore.Functions.Notify("¡Parece que ya has pasado tu prueba! ve al Ayuntamiento para obtener una nueva Licencia", "error", 10000)
                else
...
RegisterNetEvent('opendriving:theory', function()
    QBCore.Functions.TriggerCallback('qb-drivingschool:server:hasfunds', function(HasFunds)
        if HasFunds then
            ...
            QBCore.Functions.Notify("¡Parece que no tienes suficientes fondos en tu banco!")
        end
    end)
end)
...
RegisterNetEvent('qb-drivingschool:client:starttest', function()
    QBCore.Functions.TriggerCallback('qb-drivingschool:server:HasPermit', function(HasItem)
        if HasItem then
            ...
            QBCore.Functions.Notify("No puede cerrar la pantalla hasta que dé una prueba teórica", "error")
            StartTheoryTest()
        end
    end)
end)