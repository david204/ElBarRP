...
RegisterNetEvent('nc-backpack:server:saveBackpack', function(source, stashId, items, cb)
     ...
     QBCore.Functions.CreateUseableItem(item_name, function(source, item)
          ...
          if item.info == '' or (type(item.info) == "table" and item.info.ID == nil) then
               ...
               if value.locked then
                    TriggerClientEvent('nc-backpack:client:create_password', source, metadata.ID)
               end
               return
          end
          ...
          if isOnHotbar(item.slot) then
               TriggerClientEvent('nc-backpack:client:enter_password', source, metadata)
          else
...
QBCore.Functions.CreateUseableItem('briefcaselockpicker', function(source, item)
     ...
     for _, name in pairs(picables) do
          ...
          TriggerClientEvent('nc-backpack:client:lockpick', source, metadata)
...
RegisterNetEvent('nc-backpack:server:add_password', function(data)
     local Player = QBCore.Functions.GetPlayer(source)
...
RegisterNetEvent('nc-backpack:server:open_backpack', function(backpack_metadata, lockpick)
     ...
     if not backpack_metadata.locked then
          TriggerClientEvent('nc-backpack:client:open', backpack_metadata.source, safe_data)
          return
     end
...
QBCore.Functions.CreateCallback('nc-backpack:server:UpdateWeight', function(source, cb, ID)
     local src = source
