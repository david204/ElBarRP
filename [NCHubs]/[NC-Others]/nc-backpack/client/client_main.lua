...
local function open_stash(metadata)
     ...
     TriggerEvent("nc-backpack:client:close", metadata.ID)
end

RegisterNetEvent("nc-backpack:client:open", function(backpack_metadata)
...
local function close_stash(ID)
     QBCore.Functions.TriggerCallback('nc-backpack:server:UpdateWeight', function(weight)
...
RegisterNetEvent("nc-backpack:client:close", function(ID)
...
RegisterNetEvent('nc-backpack:client:enter_password', function(backpack_metadata)
     if backpack_metadata.locked then
          ...
          if inputData then
               ...
               TriggerServerEvent('nc-backpack:server:open_backpack', backpack_metadata)
          end
     else
          TriggerServerEvent('nc-backpack:server:open_backpack', backpack_metadata)
     end
...
RegisterNetEvent('nc-backpack:client:create_password', function(ID)
     local inputData = exports['qb-input']:ShowInput({
          header = 'Password',
          inputs = {
               {
     ...
     if inputData then
          if not inputData.pass then return end
          TriggerServerEvent('nc-backpack:server:add_password', { ID = ID, password = inputData.pass })
     end
end)

RegisterNetEvent("nc-backpack:client:lockpick", function(backpack_metadata)
     local duration = Config.duration.close
     QBCore.Functions.Progressbar("keep_backpack_close", 'Lockpicking', 1 * 1000,
          ...
          TriggerServerEvent('nc-backpack:server:open_backpack', backpack_metadata, true)
     end)
end)
          ...
          if isItemBackpack(item.name) then
               ...
               elseif backpack.cloth then
                    if BODY.bones['Back'].current_active_porp then
                         TriggerEvent('nc-clothing:client:loadOutfit', {
                              outfitData = {
          ...
          if item ~= 'empty' and isItemBackpack(item.name) then
               ...
               elseif backpack.cloth then
                    if not BODY.bones['Back'].current_active_porp then
                         TriggerEvent('nc-clothing:client:loadOutfit', { outfitData = backpack.cloth })
