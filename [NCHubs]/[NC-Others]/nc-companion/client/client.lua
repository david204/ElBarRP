...
function ActivePed:remove(index)
    ...
    TriggerServerEvent('nc-companion:server:ForceRemoveNetEntity', netId)
    self.data[index] = nil
...
function ActivePed:removeAll()
    ...
    for key, value in pairs(ActivePed:petsList()) do
        ...
        TriggerServerEvent('nc-companion:server:updateAllowedInfo', currentItem, {
            key = 'XP',
        })
    end
    TriggerServerEvent('nc-companion:server:onPlayerUnload', tmpHash)
    self.data = {}
    self.onControl = -1
end
...
RegisterNetEvent('nc-companion:client:callCompanion')
AddEventHandler('nc-companion:client:callCompanion', function(modelName, hostileTowardPlayer, item)
    ...
    CoreName.Functions.Progressbar("callCompanion", "Calling companion", Config.Settings.callCompanionDuration * 2000,
        false, false, {
        ...
        QBCore.Functions.TriggerCallback('nc-companion:server:updatePedData', function(result)
            ...
            if petData.hostile == true then
                TriggerServerEvent('nc-companion:server:despwan_not_owned_pet', petData.itemData.info.hash)
                return
            end
...
function request_healing_process(ped, item, process_type)
    ...
    CoreName.Functions.Progressbar("reviveing", "Reviveing",
        ...
        TriggerServerEvent('nc-companion:server:revivePet', current_pet, process_type)
        TaskFollowTargetedPlayer(ped, plyID, false)
    end)
end

RegisterNetEvent('nc-companion:client:update_health_value', function(item, amount)
    ...
    CreateThread(function()
        ...
        while DoesEntityExist(ped) and fninished == false do
            ...
            if IsPedDeadOrDying(savedData.entity) == false and savedData.maxHealth ~= currentHealth and
                ...
                currentHealth then
                -- ped is still alive
                TriggerServerEvent('nc-companion:server:updateAllowedInfo', {
...
RegisterNetEvent('nc-companion:client:forceKill', function(hash, reason)
...
RegisterNetEvent('nc-companion:client:despawn')
AddEventHandler('nc-companion:client:despawn', function(item, revive)
    if revive ~= nil and revive == true then
        ...
        TriggerServerEvent('nc-companion:server:setAsDespawned', item)
        return
    end
    local plyPed = PlayerPedId()
...
RegisterNetEvent('nc-companion:client:start_feeding_animation', function()
    ...
    CoreName.Functions.Progressbar("feeding", "Feeding", Config.core_items.food.settings.duration * 1000, false, false,
        ...
        TriggerServerEvent('nc-companion:server:increaseFood', current_pet.itemData)
    end)
end)

RegisterNetEvent('nc-companion:client:', function()
...
function start_drinking_animation()
    ...
    CoreName.Functions.Progressbar("pet_drinking", "drinking", Config.core_items.waterbottle.settings.duration * 1000,
        ...
        QBCore.Functions.TriggerCallback('nc-companion:server:decrease_thirst', function(result)

        end, current_pet.itemData)
    end)
end

RegisterNetEvent('nc-companion:client:filling_animation', function(item)
    CoreName.Functions.Progressbar("filling_animation", "filling bottle",
        ...
        TriggerServerEvent('nc-companion:server:filling_event', item)
    end)
end)

RegisterNetEvent('nc-companion:client:rename_name_tag', function(item)
    ...
    if name then
        ...
        TriggerServerEvent('nc-companion:server:rename_name_tag', name.pet_name)
    end
end)



RegisterNetEvent('nc-companion:client:rename_name_tagAction', function(name)
    ...
    CoreName.Functions.Progressbar("waitingForName", "waiting for Name",
        ...
        QBCore.Functions.TriggerCallback('nc-companion:server:renamePet', function(result)
...
RegisterNetEvent('nc-companion:client:collar_process', function()
    ...
    if inputData then
        ...
        CoreName.Functions.Progressbar("waitingForOwenership", "waiting for new owner",
            ...
            function()
                ...
                QBCore.Functions.TriggerCallback('nc-companion:server:collar_change_owenership', function(result)
