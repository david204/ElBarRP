...
function k9SearchVehicle(veh, activePed)
    ...
    for key, value in pairs(coo) do
        ...
        Wait(2000)
        QBCore.Functions.TriggerCallback('nc-companion:server:search_vehicle', function(result)
            if result then
...
-- Command
AddEventHandler('nc-companion:client:actionMenuDispatcher', function(option)
...
AddEventHandler('nc-companion:client:main_menu', function()
    ...
    local openMenu = { {
        ...
        params = {
            event = "nc-companion:client:action_menu"
        }
    }, {
        ...
        params = {
            event = "nc-companion:client:switchControl_menu"
        }
    }, {
...
AddEventHandler('nc-companion:client:action_menu', function()
    local name = ActivePed.read().itemData.info.name
    ...
    local openMenu = {
        {
            ...
            params = {
                event = "nc-companion:client:main_menu",
            }
        },
        {
            header = header,
    ...
    for key, value in ipairs(menu) do
        ...
        openMenu[#openMenu + 1] = {
            ...
            txt = value.desc or "",
            params = {
                event = "nc-companion:client:actionMenuDispatcher",
                args = {
                    type = value.TYPE,
                    menu = menu
                }
    ...
    openMenu[#openMenu + 1] = {
        ...
        txt = "",
        params = {
            event = "nc-companion:client:tricks_menu"
        }
    }
...
AddEventHandler('nc-companion:client:tricks_menu', function()
    local name = ActivePed.read().itemData.info.name
...
AddEventHandler('nc-companion:client:switchControl_menu', function()
    local name = ActivePed.read().itemData.info.name
    ...
    for key, value in pairs(ActivePed:petsList()) do
        openMenu[#openMenu + 1] = {
            ...
            icon = 'fa-solid fa-' .. key,
            params = {
                event = "nc-companion:client:switchControl_event",
                args = {
                    index = value.key
                }
            }
...
AddEventHandler('nc-companion:client:switchControl_event', function(option)
    if option.index < 0 then
        return
    end
    ActivePed:switchControl(option.index)
    TriggerEvent('nc-companion:client:action_menu')
end)
...
RegisterCommand('+showMenu', function()
    ...
    if ((IsDowned() and IsPoliceOrEMS()) or not IsDowned()) and not Ishandcuffed() and not IsPauseMenuActive() and
        ...
        TriggerEvent('nc-companion:client:main_menu')
    end
end, false)
...
--           Customization menu
-- =======================================
RegisterNetEvent('nc-companion:client:start_grooming_process', function()
    local activePed = ActivePed:read()
    if type(activePed) ~= "table" then
        QBCore.Functions.Notify(Lang:t('error.no_pet_under_control'), 'error', 5000)
        return
    end
    TriggerServerEvent('nc-companion:server:grooming_process', activePed.itemData)
end)

RegisterNetEvent('nc-companion:client:initialization_process', function(item, pet_information)
    if type(item) ~= "table" then
        QBCore.Functions.Notify(Lang:t('error.failed_to_start_procces'), 'error', 5000)
        return
    end
    if pet_information.type == 'init' then
        TriggerEvent('nc-companion:client:openMenu_customization', {
...
AddEventHandler('nc-companion:client:openMenu_customization', function(data)
    openMenu_customization(data)
end)

AddEventHandler('nc-companion:client:openMenu_customization_rename', function(data)
    ...
    if inputData then
        ...
        if type(validation) == "table" and next(validation) ~= nil then
            ...
            if validation.reason == 'badword' then
                ...
                TriggerEvent('nc-companion:client:openMenu_customization_rename', data)
                return
...
AddEventHandler('nc-companion:client:openMenu_customization_rename:rename', function(data)
    rename(data)
end)

AddEventHandler('nc-companion:client:openMenu_customization_select_variation', function(data)
    ...
    local openMenu = {
        {
            ...
            icon = 'fa-solid fa-angle-left',
            params = {
                event = "nc-companion:client:openMenu_customization_select_variation",
                args = {
                    item = item,
...
AddEventHandler('nc-companion:client:openMenu_customization_variation:variation_menu', function(data)
...
function openMenu_customization(data)
    ...
    local openMenu = {
        ...
        },
        {
            ...
            params = {
                event = "nc-companion:client:openMenu_customization_rename",
                args = {
                    item = data.item,
        ...
        },
        {
            ...
            params = {
                event = "nc-companion:client:openMenu_customization:confirm",
                args = {
                    item = data.item,
                    pet_information = data.pet_information
                }
    ...
    local openMenu = {
        {
            ...
            icon = 'fa-solid fa-angle-left',
            params = {
                event = "nc-companion:client:openMenu_customization",
                args = {
                    item = item,
                    pet_information = pet_information
                },
        ...
        },
        {
            ...
            icon = 'fa-regular fa-keyboard',
            params = {
                event = "nc-companion:client:openMenu_customization_rename:rename",
                args = {
                    item = item,
                    pet_information = pet_information
                },
...
function openMenu_customization_select_variation(data)
    ...
    local openMenu = {
        ...
        },
        {
            ...
            icon = 'fa-solid fa-brush',
            params = {
                event = "nc-companion:client:openMenu_customization_variation:variation_menu",
                args = {
                    item = item,
                    pet_information = pet_information
                }
...
AddEventHandler('nc-companion:client:openMenu_customization:confirm', function(data)
    TriggerServerEvent('nc-companion:server:compelete_initialization_process', data.item, data.pet_information.type)
end)
