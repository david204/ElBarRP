...
function initItem(source, item)
    ...
    if Config.Settings.let_players_cutomize_their_pet_after_purchase then
        ...
        TriggerClientEvent('nc-companion:client:initialization_process', src, item, information)
    end
end
...
RegisterNetEvent('nc-companion:server:compelete_initialization_process', function(item, process_type)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player then return end

    TriggerEvent('nc-companion:server:nc-companion:server:compelete_initialization_process_last_step', source, item,
        Player, process_type)
    if process_type == 'init' then return end
    Player.Functions.RemoveItem(Config.core_items.groomingkit.item_name, 1)
end)

RegisterNetEvent('nc-companion:server:nc-companion:server:compelete_initialization_process_last_step',
...
QBCore.Functions.CreateCallback('nc-companion:server:collar_change_owenership', function(source, cb, data)
