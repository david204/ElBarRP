...
function Pet:spawnPet(source, model, item)
    ...
    TriggerClientEvent('nc-companion:client:callCompanion', source, model, owner, item)
end

RegisterNetEvent('nc-companion:server:despwan_not_owned_pet', function(hash)
...
function Pet:despawnPet(source, item, revive)
    ...
    TriggerClientEvent('nc-companion:client:despawn', source, item, revive)
end
...
function Pet:save_all_info(source, hash)
    ...
    if petData.info.health > 100 then
        ...
        TriggerClientEvent('nc-companion:client:forceKill', source, hash, 'hunger')
    end
...
RegisterNetEvent('nc-companion:server:setAsDespawned', function(item)
...
QBCore.Functions.CreateUseableItem(core_items.food.item_name, function(source, item)
    local Player = QBCore.Functions.GetPlayer(source)
    if Player == nil then return end
    TriggerClientEvent('nc-companion:client:start_feeding_animation', source)
end)

RegisterNetEvent('nc-companion:server:increaseFood', function(item)
    local Player = QBCore.Functions.GetPlayer(source)
...
QBCore.Functions.CreateUseableItem(core_items.collar.item_name, function(source, item)
    TriggerClientEvent('nc-companion:client:collar_process', source)
end)
...
QBCore.Functions.CreateUseableItem(core_items.nametag.item_name, function(source, item)
    local Player = QBCore.Functions.GetPlayer(source)
    if Player == nil or item == nil then return end
    TriggerClientEvent('nc-companion:client:rename_name_tag', source, item)
end)

RegisterNetEvent('nc-companion:server:rename_name_tag', function(name)
    local Player = QBCore.Functions.GetPlayer(source)
    if Player == nil then return end
    ...
    TriggerClientEvent("nc-companion:client:rename_name_tagAction", source, name)
end)
...
QBCore.Functions.CreateUseableItem(core_items.groomingkit.item_name, function(source, item)
    local Player = QBCore.Functions.GetPlayer(source)
    if Player == nil then return end
    TriggerClientEvent('nc-companion:client:start_grooming_process', source)
end)

RegisterNetEvent('nc-companion:server:grooming_process', function(item)
    ...
    TriggerClientEvent('nc-companion:client:initialization_process', source, item, information)
end)
...
QBCore.Functions.CreateUseableItem(core_items.waterbottle.item_name, function(source, item)
    ...
    TriggerClientEvent('nc-companion:client:filling_animation', source, item)
end)

RegisterNetEvent('nc-companion:server:filling_event', function(item)
    fillwater_bottle(source, item)
end)

QBCore.Functions.CreateCallback('nc-companion:server:decrease_thirst', function(source, cb, data)
    local player = QBCore.Functions.GetPlayer(source)
    ...
    if pet_water_bottle.info == nil then
        ...
        print('issue with nill info: https://github.com/swkeep/nc-companion/issues/25')
        return
    end
    ...
    if pet_water_bottle.info.liter == nil then
        ...
        print('developer: issue with nill info -> liter: https://github.com/swkeep/nc-companion/issues/25')
        return
    end
...
RegisterNetEvent('nc-companion:server:revivePet', function(item, process_type)
    local src = source
    ...
    if process_type and process_type == 'Heal' then
        ...
        TriggerClientEvent('nc-companion:client:update_health_value', src, item, petData.info.health)
...
QBCore.Functions.CreateCallback('nc-companion:server:search_inventory', function(source, cb, cid)
...
QBCore.Functions.CreateCallback('nc-companion:server:search_vehicle', function(source, cb, data)
...
RegisterNetEvent('nc-companion:server:updateAllowedInfo', function(item, data)
...
QBCore.Functions.CreateCallback('nc-companion:server:renamePet', function(source, cb, item)
    local player = QBCore.Functions.GetPlayer(source)
    local current_pet_data = Pet:findbyhash(source, item.hash)
...
QBCore.Functions.CreateCallback('nc-companion:server:updatePedData', function(source, cb, clientRes)
    local player = QBCore.Functions.GetPlayer(source)
    if player == nil then
        cb(false)
        return
...
RegisterNetEvent('nc-companion:server:onPlayerUnload', function(items)
...
QBCore.Commands.Add('renamePet', 'rename pet', { { "name", "new pet name" } }, false, function(source, args)
    TriggerClientEvent("nc-companion:client:rename_name_tag", source, args[1])
end, 'admin')
...
RegisterNetEvent('nc-companion:server:ForceRemoveNetEntity', function(netId)
