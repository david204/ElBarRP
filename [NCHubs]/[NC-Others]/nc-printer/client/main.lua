...
RegisterNetEvent('nc-printer:client:UsePaperDocument', function(ItemData)
...
RegisterNetEvent('nc-printer:client:useprinterbox', function()
...
RegisterNUICallback('SaveDocument', function(data)
    if data.url then
        TriggerServerEvent('nc-printer:server:SavePaperDocument', data.url)
    end
end)
...
exports["qb-target"]:AddBoxZone("gabzpdmprinter", vector3(-37.69, -1086.62, 26.67), 1.3, 1.5, {
	...
	options = {
		{
			event = "nc-printer:client:useprinterbox",
			icon = "fas fa-print",
