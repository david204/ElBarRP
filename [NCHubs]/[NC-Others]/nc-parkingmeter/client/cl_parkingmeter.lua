...
RegisterNetEvent('nc-parkingmeter:client:payParking', function()
    ...
    QBCore.Functions.Progressbar("drilling", "Making payment...", 3000, false, true, {
        ...
        QBCore.Functions.TriggerCallback('nc-parkingmeter:server:checkParking', function(isPaid)
            if isPaid then
                QBCore.Functions.Notify('Parking is already paid!', 'error', 5000)
            else
                TriggerServerEvent('nc-parkingmeter:server:payParking', pos)
            end
        end, pos)
    end, function()
        TriggerEvent('animations:client:EmoteCommandStop')
...
RegisterNetEvent('nc-parkingmeter:client:checkParking', function()
    local pos = GetEntityCoords(PlayerPedId())
    TriggerEvent('animations:client:EmoteCommandStart', {"parkingmeter"})
