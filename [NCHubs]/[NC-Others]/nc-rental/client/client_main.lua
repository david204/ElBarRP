            ...
            options = {
                {
                    ...
                    action = function()
                        TriggerEvent('nc-rental:client:RentalMenu', Spot, v.VehicleSpawnLocation)
                    end
                },
            },
            distance = 2.5
...
RegisterNetEvent('nc-rental:client:RentalMenu', function(Spot, VehicleSpawnLocation)
    if IsRenting == false then
        ...
        for k, v in pairs(Config.RentalSpots[Spot]["AvailableVehicles"]) do
            ...
            item.params = {
                event = "nc-rental:client:RentVehicleInput",
                args = {
        ...
        local columns = {
            ...
            },
            {
                ...
                icon = "fas fa-ban",
                params = {
                    event = "nc-rental:client:CancelRenting",
                }
            },
        }
        exports[Config.QBMenuName]:openMenu(columns)
...
RegisterNetEvent('nc-rental:client:RentVehicleInput', function(data)
    if IsRenting == false then
        ...
        if dialog ~= nil then
            exports[Config.QBMenuName]:openMenu({
                ...
                },
                {
                    ...
                    icon = "fas fa-check",
                    params = {
                        event = "nc-rental:client:RentVehicle",
                        args = {
...
RegisterNetEvent('nc-rental:client:RentVehicle', function(data)
    ...
    QBCore.Functions.TriggerCallback("nc-rental:server:VerifyMoney", function(hasMoney)
        if (hasMoney) then
            if IsRenting == false then
                QBCore.Functions.SpawnVehicle(vehicle, function(veh)
                    ...
                    TriggerServerEvent('nc-rental:server:RentVehicle', RentPrice, data.VehicleLableName, GetVehicleNumberPlateText(veh), data.TimeAmount,PlayerData)
                    Vehicle = veh
                end, coords, true)
                IsRenting = true
                    ...
                    if IsRenting == true then
                        ...
                        CreateThread(function()
                            while IsRenting == true do
                                ...
                                if IsRenting == true then
                                TriggerServerEvent('nc-rental:server:BillPlayer', FeesPrice)
                                TriggerEvent('nc-rental:client:Mail2')
                                end
                            end
                        end)
                    end
...
RegisterNetEvent('nc-rental:client:CancelRenting', function(data)
    print(Vehicle)
    print(IsRenting)
    if IsRenting == true then
        IsRenting = false
        ...
        TriggerServerEvent('nc-rental:server:RemoveContract')
    else 
        QBCore.Functions.Notify(Lang.NotRenting, 'error', 7500)
    end
end)

RegisterNetEvent('nc-rental:client:Mail2', function()
    if Config.PhoneName == 'qb-phone' then
        TriggerServerEvent('qb-phone:server:sendNewMail', {
            sender = Lang.MailSender,
            subject = Lang.MailSubject,
