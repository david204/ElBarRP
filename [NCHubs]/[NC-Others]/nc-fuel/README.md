...
Here, we shall provide a step-by-step guide on installing nc-fuel to your server and making it work with other scripts you may already have installed.

### Step 1:

First, we will start by renaming the resource "nc-fuel-main" to just "nc-fuel". <br> <br> Next, we will drag the "nc-fuel" resource into your desired folder in your servers resources directory.

![Step 1](https://i.imgur.com/8kg0LWe.gif)

### Step 2:

Next, we're going to drag the sounds from the *nc-fuel/assets/sounds* folder in nc-fuel, into your interact-sounds folder located at *resources/[standalone]/interact-sound/client/html/sounds*

![Step 2](https://i.imgur.com/4Mox2wP.gif)

### Step 3:

Next, we're going to open our entire resources folder in whichever IDE you use, (we will be using Visual Studio Code for this example) and replace all of your current exports titled "LegacyFuel", "ps-fuel" or "lj-fuel", with "nc-fuel". Then you want to ensure nc-fuel in your server's config file. 
<br> <br>
...
<br> <br>
The first step of installing our items is to navigate to your *nchub/shared/items.lua*.
<br> <br>
...
<br> 
```
        } else if (itemData.name == "syphoningkit") { // Syphoning Kit (nc-Fuel or nc-Syphoning!)
        ...
        } else if (itemData.name == "jerrycan") { // Jerry Can (nc-Fuel!)
            $(".item-info-title").html("<p>" + itemData.label + "</p>");
            $(".item-info-description").html(
...
<br> <br> 
Navigate to the nc-fuel resource and follow this path: *nc-fuel/assets/images*
<br> <br> 
...
```
            {
				type = "client",
				event = "nc-fuel:client:RefuelMenu",
...
Now, set the *Config.FuelTargetExport* in *nc-fuel/shared/config.lua* to **true**.
...
Enjoy using **nc-fuel**, if you have an issues, create an issue on the repository, and we will fix it **ASAP**!
...
#### Some features to mention within nc-fuel:
...
- [nc-Syphoning](https://github.com/CodineDev/nc-syphoning) built-in via Config Options.
...
Future Plans are located on the [Wiki Section](https://github.com/CodineDev/nc-fuel/wiki) of this repository, labeled [nc-Fuel Roadmap](https://github.com/CodineDev/nc-fuel/wiki).
