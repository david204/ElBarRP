...
if Config.RenewedPhonePayment then
	RegisterNetEvent('nc-fuel:server:phone:givebackmoney', function(amount)
		local src = source
...
RegisterNetEvent("nc-fuel:server:OpenMenu", function(amount, inGasStation, hasWeapon, purchasetype)
	local src = source
	...
	if inGasStation == true and not hasWeapon then
		if Config.RenewedPhonePayment and purchasetype == "bank" then
			TriggerClientEvent("nc-fuel:client:phone:PayForFuel", src, fuelamounttotal)
		else
			TriggerClientEvent('qb-menu:client:openMenu', src, {
				...
				},
				{
					...
					params = {
						event = "nc-fuel:client:RefuelVehicle",
						args = {
...
RegisterNetEvent("nc-fuel:server:PayForFuel", function(amount, purchasetype)
	local src = source
	if not src then return end
	local player = QBCore.Functions.GetPlayer(src)
	if not player then return end
...
RegisterNetEvent("nc-fuel:server:purchase:jerrycan", function(purchasetype)
...
if Config.UseJerryCan then
	QBCore.Functions.CreateUseableItem("jerrycan", function(source, item)
		local src = source
		TriggerClientEvent('nc-fuel:jerrycan:refuelmenu', src, item)
	end)
end
...
if Config.UseSyphoning then
	QBCore.Functions.CreateUseableItem("syphoningkit", function(source, item)
		local src = source
		TriggerClientEvent('nc-syphoning:syphon:menu', src, item)
	end)
end

RegisterNetEvent('nc-fuel:info', function(type, amount, srcPlayerData, itemdata)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent('nc-syphoning:callcops', function(coords)
    TriggerClientEvent('nc-syphoning:client:callcops', -1, coords)
end)
