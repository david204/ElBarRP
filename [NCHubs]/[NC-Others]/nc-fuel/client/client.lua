...
-- Client Events
RegisterNetEvent('nc-fuel:client:RefuelMenu', function()
	if Config.RenewedPhonePayment then
		if not RefuelPossible then 
			TriggerEvent('nc-fuel:client:SendMenuToServer') 
		else 
			if Config.RenewedPhonePayment then
				if not Cancelledrefuel and not RefuelCancelled then
					if RefuelPossibleAmount then
						...
						TriggerEvent('nc-fuel:client:RefuelVehicle', purchasetype, fuelamounttotal) 
					else
...
RegisterNetEvent('nc-fuel:client:grabnozzle', function()
	local ped = PlayerPedId()
...
RegisterNetEvent('nc-fuel:client:returnnozzle', function()
	holdingnozzle = false
	TargetCreated = false
...
if Config.RenewedPhonePayment then
	RegisterNetEvent('nc-fuel:client:phone:PayForFuel', function(amount)
		...
		if success then
			if QBCore.Functions.GetPlayerData().money['bank'] <= (GlobalTax(amount) + amount) then
				...
				TriggerServerEvent('nc-fuel:server:PayForFuel', cost, "bank")
				RefuelPossible = true
...
RegisterNetEvent('nc-fuel:client:FinalMenu', function(purchasetype)
	local money = nil
	if purchasetype == "bank" then money = QBCore.Functions.GetPlayerData().money['bank'] elseif purchasetype == 'cash' then money = QBCore.Functions.GetPlayerData().money['cash'] end
	...
	if fuel then
		...
		if (fuel.amount + finalfuel) >= 100 then
			...
			if GlobalTax(fuel.amount * Config.CostMultiplier) + (fuel.amount * Config.CostMultiplier) <= money then
				...
				TriggerServerEvent('nc-fuel:server:OpenMenu', totalcost, inGasStation, false, purchasetype)
			else
...
RegisterNetEvent('nc-fuel:client:SendMenuToServer', function()
	local vehicle = QBCore.Functions.GetClosestVehicle()
	...
	if CurFuel < 95 then
		exports['qb-menu']:openMenu({
			...
			},
			{
				...
				icon = "fas fa-usd",
				params = {
					event = "nc-fuel:client:FinalMenu",
					args = 'cash',
				}
			},
			{
...
RegisterNetEvent('nc-fuel:client:RefuelVehicle', function(data)
		...
		if isCloseVeh() then
				...
				while refueling do
					...
					if Cancelledrefuel then
						...
						if Config.RenewedPhonePayment and purchasetype == "bank" then
							...
							TriggerServerEvent("nc-fuel:server:phone:givebackmoney", MoneyToGiveBack)
						else
							TriggerServerEvent('nc-fuel:server:PayForFuel', refillCostB4Tax, purchasetype)
						end
						local curfuel = GetFuel(vehicle)
			...
			QBCore.Functions.Progressbar("refuel-car", "Refueling", time, false, true, {
				...
				function()
					SetBusy(false)
					refueling = false
					if not Config.RenewedPhonePayment then TriggerServerEvent('nc-fuel:server:PayForFuel', refillCost, purchasetype) end
					local curfuel = GetFuel(vehicle)
	...
	options = {
		{
			num = 1,
			type = "client",
			event = "nc-fuel:client:grabnozzle",
			icon = "fas fa-gas-pump",
			label = "Grab Nozzle",
		...
		},
		{
			num = 2,
			type = "client",
			event = "nc-fuel:client:purchasejerrycan",
		...
		},
		{
			...
			type = "client",
			event = "nc-fuel:client:returnnozzle",
			icon = "fas fa-hand",
			label = "Return Nozzle",
			canInteract = function()
		...
		options = {
			{
				type = "client",
				action = function ()
					TriggerEvent("nc-fuel:client:RefuelMenu")
				end,
				icon = "fas fa-gas-pump",
				label = "Insert Nozzle",
				canInteract = function()
...
-- Jerry Can --
RegisterNetEvent('nc-fuel:jerrycan:refuelmenu', function(itemData)
	...
	if holdingnozzle then
		...
		exports['qb-menu']:openMenu({
			...
			},
			{
				...
				icon = "fas fa-gas-pump",
				params = {
					event = "nc-fuel:jerrycan:refueljerrycan",
					args = {
						itemData = itemData,
					},
				},
		...
		exports['qb-menu']:openMenu({
			...
			},
			{
				...
				icon = "fas fa-gas-pump",
				params = {
					event = "nc-fuel:jerrycan:refuelvehicle",
					args = {
						itemData = itemData,
					},
				},
...
RegisterNetEvent('nc-fuel:client:jerrycanfinalmenu', function(purchasetype)
	Moneyamount = nil
	...
	if Moneyamount > math.ceil(Config.JerryCanPrice + GlobalTax(Config.JerryCanPrice)) then
		TriggerServerEvent('nc-fuel:server:purchase:jerrycan', purchasetype)
	else
...
RegisterNetEvent('nc-fuel:client:purchasejerrycan', function()
	local playercashamount = QBCore.Functions.GetPlayerData().money['cash']
	exports['qb-menu']:openMenu({
		{
		...
		},
		{
			...
			icon = "fas fa-usd",
			params = {
				event = "nc-fuel:client:jerrycanfinalmenu",
				args = 'cash',
			}
		},
		{
...
RegisterNetEvent('nc-fuel:jerrycan:refuelvehicle', function(data)
	local vehicle = QBCore.Functions.GetClosestVehicle()
	...
	if refuel then
		...
		function() -- Play When Done
			...
			TriggerServerEvent('nc-fuel:info', "remove", tonumber(refuel.amount), srcPlayerData, syphonData)
...
RegisterNetEvent('nc-fuel:jerrycan:refueljerrycan', function(data)
	local itemData = data.itemData
	...
	if refuel then
		...
		function() -- Play When Done
			...
			local syphonData = data.itemData
			local srcPlayerData = QBCore.Functions.GetPlayerData()
			TriggerServerEvent('nc-fuel:info', "add", tonumber(refuel.amount), srcPlayerData, syphonData)
			TriggerServerEvent('nc-fuel:server:PayForFuel', tonumber(refuel.amount) * Config.CostMultiplier, "cash")
		end, function() -- Play When Cancel
			SetEntityVisible(fuelnozzle, true, 0)
			SetBusy(false)
			DeleteObject(JerrycanProp)
...
local function PoliceAlert(coords)
	...
	if chance < Config.SyphonPoliceCallChance then
		...
		elseif Config.SyphonDispatchSystem == "nc-dispatch" then
			TriggerServerEvent('nc-dispatch:911call', coords)
		elseif Config.SyphonDispatchSystem == "nc-default" then
			TriggerServerEvent('nc-syphoning:callcops', coords)
...
-- Events --
RegisterNetEvent('nc-syphoning:syphon:menu', function(itemData)
...
RegisterNetEvent('nc-syphoning:syphon', function(data)
			...
			if syphon then
				...
				if (tonumber(syphon.amount) <= tonumber(cargasamount)) then
					...
					function() -- Play When Done
						...
						local syphonData = data.itemData
						local srcPlayerData = QBCore.Functions.GetPlayerData()
						TriggerServerEvent('nc-fuel:info', "add", tonumber(syphon.amount), srcPlayerData, syphonData)
					end, function() -- Play When Cancel
						SetBusy(false)
						PoliceAlert(GetEntityCoords(PlayerPedId()))
						StopAnimTask(PlayerPedId(), Config.StealAnimDict, Config.StealAnim, 1.0)
...
RegisterNetEvent('nc-syphoning:client:callcops', function(coords)
