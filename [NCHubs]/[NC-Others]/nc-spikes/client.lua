...
AddEventHandler("spikestrips:client:usespikes", function(config)
    ...
    TriggerServerEvent("nc-spikes-use")
end)
...
RegisterCommand("RemoveSpikes",function()
    for a = 1, #SpawnedSpikes do
        Animation()
        TriggerServerEvent("nc-spikes-remove", SpawnedSpikes[a])
    end
    SpawnedSpikes = {}
end)

RegisterNetEvent("nc-spikes-delete")
AddEventHandler("nc-spikes-delete", function(netid)
    Citizen.Wait(1700)
    ClearPedTasksImmediately(PlayerPedId())
