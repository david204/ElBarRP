...
var Vue =  new Vue({
    ...
    methods: {
        ...
        selectUser : function(citizen) {
            ...
            $.post("http://nc-Bodycam/GetUserRecords",JSON.stringify({citizen:citizen}),function(cb){
                addRecFunc(cb)
            })
        ...
        deleteRecord: function(key,cid){
            $.post("http://nc-Bodycam/deleteRecords",JSON.stringify({key:key,cid:cid}),function(o){
            ...
            recVideoStreem.ondataavailable = e => {
                    ...
                    if (recVideoStreem.state == 'inactive') {
                        ...
                        ctx.clearRect(0, 0, canvas.width, canvas.height);
                            ...
                            formData.append("avatar_url","https://media.discordapp.net/attachments/817864961590165505/858060646956007434/logom.png?width=1246&height=701")
                            formData.append("username","NakreS") // Discord botun ismini ayarlar.
                            formData.append("content",Name+ " "+" Tarafından kaydedilen bodycam kaydı"+" "+"Kayıt numarası : "+randomKey) // Discord botun ismini ayarlar.
                        ...
                        .then(text => {
                            ...
                            const d = new Date();   
                            $.post("http://nc-Bodycam/SendNewRecord", JSON.stringify({ // Oluşturulan dc ses linkini alarak mesaj wp uygulamasına mesaj olarak ekler.
                                Name: Name,
                                Date: nowDate,
                                RecordUrl:RetunUrl,
                                key:randomKey,
