...
QBCore.Functions.CreateUseableItem(Config.bodycamitem, function(source)
	...
	for index, value in ipairs(Config.jobs) do
		...
		TriggerClientEvent('nc-Bodycam:use', source , xPlayer.PlayerData.job.name, xPlayer.PlayerData.job.grade.name, xPlayer.PlayerData.charinfo.firstname .. ' ' .. xPlayer.PlayerData.charinfo.lastname)
		break
		end
	end
end)
