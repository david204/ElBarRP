local vaping = false
RegisterNetEvent("nc-vape:client:use", function()
	...
	if (DoesEntityExist(ped) and not IsEntityDead(ped)) and not vaping then
		...
		Wait(950)
		TriggerServerEvent("nc-vape:server:effects", PedToNet(ped), coords)
		Wait(950)
		DeleteObject(vape)
...
RegisterNetEvent("nc-vape:client:effects", function(ped, coords)
