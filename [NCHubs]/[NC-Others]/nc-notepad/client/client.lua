...
RegisterNetEvent('QBCore:Client:OnPlayerLoaded', function() TriggerEvent("nc-notepad:Client:SyncNotes") end)
AddEventHandler('onResourceStart', function(r) if GetCurrentResourceName() ~= r then return end TriggerEvent("nc-notepad:Client:SyncNotes") end)

RegisterNetEvent("nc-notepad:Client:SyncNotes", function(newNotes)
	if not newNotes then
		local p = promise.new()
		QBCore.Functions.TriggerCallback('nc-notepad:Server:SyncNotes', function(cb) p:resolve(cb) end)
	...
	for k, v in pairs(Notes) do
		if not Props[k] and Notes[k] then
			...
			Targets[k] =
				...
				{ options = { { type = "server", event = "nc-notepad:Server:ReadNote", icon = "fas fa-receipt", label = Loc[Config.Lan].targetinfo["read"], noteid = k, }, },
						distance = 1.5 })
		end
	end
...
RegisterNetEvent("nc-notepad:Client:CreateNote", function()
    ...
    if dialog.note ~= nil then
		...
		Wait(900)
        TriggerServerEvent("nc-notepad:Server:CreateNote", {
...
RegisterNetEvent("nc-notepad:Client:ReadNote", function(data)
	local notepad = {}
	...
	notepad[#notepad+1] = { icon = "fas fa-hand-scissors", header = "", text = Loc[Config.Lan].menu["tear_up_note"], params = { event = "nc-notepad:Client:DestroyNote", args = data } }
	exports["qb-menu"]:openMenu(notepad)
end)

RegisterNetEvent("nc-notepad:Client:DestroyNote", function(data)
	loadAnimDict("pickup_object")
	TaskPlayAnim(PlayerPedId(), "pickup_object", "pickup_low", 8.0, -8.0, -1, 0, 0, false, false, false)
	unloadAnimDict("pickup_object")
	Wait(900)
	TriggerServerEvent("nc-notepad:Server:SyncEffect", data.coords)
	TriggerServerEvent("nc-notepad:Server:DestroyNote", data.id)
end)

RegisterNetEvent("nc-notepad:Client:SyncEffect", function(coords)
