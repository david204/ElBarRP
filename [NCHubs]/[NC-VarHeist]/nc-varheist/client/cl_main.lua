    ...
    QBCore.Functions.TriggerCallback('QBCore:HasItem', function(result)
        if result then
            if helmet then
                ...
                TriggerServerEvent('nc-varheist:server:ConnectDock', index, connected)
            else
...
LowerHack = function(index)
    if not Config.LowerHacks[index].busy then
        -- Set Busy
        TriggerServerEvent('nc-varheist:server:SetLowerBusy', index, true)
        ...
        exports['varhack']:OpenHackingGame(function(success)
            if success then
                ...
                TriggerServerEvent('nc-varheist:server:LowerHackResult', index)
            end
            StopAnimTask(ped, animDict, anim, 1.0)
            -- Unset Busy
            TriggerServerEvent('nc-varheist:server:SetLowerBusy', index, false)
        end, Config.LowerHacks[index].difficulty, 7)
    end
end

RegisterNetEvent('nc-varheist:client:VarEnter', function()
    local ped = PlayerPedId()
    Wait(500)
    DoScreenFadeOut(250)
    Wait(250)
    ...
    Wait(250)
    DoScreenFadeIn(250)
    TriggerServerEvent('nc-varheist:server:EnterVar')
    --TriggerEvent('nc-varheist:enter') -- use this event in other scripts
...
RegisterNetEvent('nc-varheist:client:ExitVAR', function()
    TriggerServerEvent('QBCore:CallCommand', "var_exit", nil)
end)

RegisterNetEvent('nc-varheist:client:VarExit', function()
    local ped = PlayerPedId()
    Wait(500)
    DoScreenFadeOut(500)
    Wait(250)
    ...
    Wait(250)
    DoScreenFadeIn(1000)
    --TriggerEvent('nc-varheist:leave') -- use this event in other scripts
end)

RegisterNetEvent('nc-varheist:client:UseHelmet', function()
    local ped = PlayerPedId()
...
RegisterNetEvent('nc-varheist:client:ConnectDock', function(dock, bool)
    Config.Docks[dock] = bool
end)

RegisterNetEvent('nc-varheist:client:InstructionMail', function()
    Wait(3000)
...
RegisterNetEvent('nc-varheist:client:UseMedKit', function()
...
RegisterNetEvent('nc-varheist:client:StartHack', function()
    if not Config.CurrentHack.busy then
        -- Set Busy
        TriggerServerEvent('nc-varheist:server:SetBusy', true)
        local animDict = 'anim@heists@prison_heiststation@cop_reactions'
        local anim = 'cop_b_idle'
        RequestAnimDict(animDict)
        while not HasAnimDictLoaded(animDict) do Wait(10) end
        ...
        exports['varhack']:OpenHackingGame(function(success)
            ...
            TriggerServerEvent('nc-varheist:server:HackResult', success)
            StopAnimTask(ped, animDict, anim, 1.0)
            -- Set Not Busy
            TriggerServerEvent('nc-varheist:server:SetBusy', false)
...
RegisterNetEvent('nc-varheist:client:SetBusy', function(bool)
    Config.CurrentHack.busy = bool
end)

RegisterNetEvent('nc-varheist:client:SetLowerBusy', function(index, bool)
    Config.LowerHacks[index].busy = bool
end)

RegisterNetEvent('nc-varheist:client:EnableServer', function(server)
    exports["qb-target"]:AddCircleZone("VARServer", Config.Servers[server].coords, 0.5, {
        ...
        options = {
            {
                type = "client",
                event = "nc-varheist:client:InsertServer",
...
RegisterNetEvent('nc-varheist:client:InsertServer', function()
    ...
    QBCore.Functions.Progressbar('var_server', 'Configuring Server Panel', 20000, false, true, {
        ...
        TriggerServerEvent('nc-varheist:server:GetPassword')
...
RegisterNetEvent('nc-varheist:client:RemoveServerTarget', function()
    exports['qb-target']:RemoveZone("VARServer")
end)

RegisterNetEvent('nc-varheist:client:SetHacked', function()
    Config.CurrentHack.hacked = true
end)

RegisterNetEvent('nc-varheist:client:EnterPassword', function()
    if not Config.CurrentHack.busy then
        -- Set Busy
        TriggerServerEvent('nc-varheist:server:SetBusy', true)
        local animDict = 'anim@heists@prison_heiststation@cop_reactions'
        ...
        if keyboard then
            QBCore.Functions.TriggerCallback('nc-varheist:server:EnterPassword',function(correct)
                if correct then
                    PlaySound(-1, "Lose_1st", "GTAO_FM_Events_Soundset", 0, 0, 1)
                end
            end, input)
...
RegisterNetEvent('nc-varheist:client:SetPassword', function(bool)
...
RegisterNetEvent('nc-varheist:client:LowerHackResult', function(index)
    Config.LowerHacks[index].hacked = true
end)

RegisterNetEvent('nc-varheist:client:Shutdown', function()
...
RegisterNetEvent('nc-varheist:client:ResetVarHeist', function()
