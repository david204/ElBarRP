...
RemoveGuards = function()
    ...
    spawnedGuards = {}
    if debug then
        print('^3[nc-varheist] ^5Deleted All Guards^7')
    end
end
...
SpawnGuards = function() -- You can always slot more guards in here
    ...
    TriggerClientEvent('nc-varheist:client:SpawnGuards', random, netIds)
    if debug then
        print('^3[nc-varheist] ^5Guards: New Wave^7')
    end
    SetTimeout(3*60*1000, SpawnGuards) -- 3 min interval to spawn guards
end

RegisterNetEvent('nc-varheist:server:LootGuards', function(netId)
    local src = source
