...
$(document).on('click', ".color", function() {
	...
	$.post("https://nc-vehicleshopui/changeColor",JSON.stringify({colorR,colorG,colorB}));
});
$(document).on('click', ".category, .category5", function() {
	...
	$("#title-stock").text("stock: " + stock);
	$("#title-price").text("price: " + costs.toLocaleString() + "$");
	$("#buy-btn").fadeIn();
	$("#test-btn").fadeIn();
	$("#hide-button").fadeIn();
	...
	$.post("https://nc-vehicleshopui/spawnVehicle",JSON.stringify({model}));
});
...
$("#test-accept").click(function() {
	let timer = data.testDrive.testDriveTimer;
	$.post("https://nc-vehicleshopui/testDrive",JSON.stringify({timer,details}))
})
...
$("#buy").click(function() {
	...
	$.post("https://nc-vehicleshopui/buyVehicle",JSON.stringify({details}));
})
	...
	if (key == "Escape"){
		if(!onBuyPage){
			if(vehiclesCategory){
				...
				$.post("https://nc-vehicleshopui/deletevehicle",JSON.stringify({}));
			}else{
				$("#vehicle-class-title-container").fadeOut();
				$("#vehicle-class-container").fadeOut();
				$.post("https://nc-vehicleshopui/closeVehicleShop",JSON.stringify({}));
			}
		}
...
$("#hide-button").click(function(){
	if(!hideDisplay){
		...
		$.post("https://nc-vehicleshopui/zoomCam",JSON.stringify({}));
	}else{
		console.log("dsa")
		hideDisplay = false;
		inspect = false;
		$("#hide-button").css("margin-top","2px");
		$.post("https://nc-vehicleshopui/returnCam",JSON.stringify({}));
		setTimeout(() => {
			$("#vehicle-container").fadeIn();
			$("#title-container").fadeIn();
...
//     $.post("https://nc-vehicleshopui/changePos",JSON.stringify({data}));
// }
...
document.onmousedown = function(e){
	if(inspect){
		if (vehicleDisplay && !down && e.button == 0){
			down = true;
			$.post("https://nc-vehicleshopui/mousedown",JSON.stringify({}));
		}
	}
}
...
document.onmouseup = function(e){
	if(inspect){
		if (down && e.button == 0){
			$.post("https://nc-vehicleshopui/mouseup",JSON.stringify({}))
			down = false;
		}
	}
}
...
window.onmousewheel = function(e){
	if(inspect){
		...
		$.post("https://nc-vehicleshopui/" + (st < 0 ? "downscroll" : "upscroll"),JSON.stringify({}));
	}
}
