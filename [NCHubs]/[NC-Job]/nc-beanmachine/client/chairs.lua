	...
	if Config.Gabz then
		for k, v in pairs(Config.GabzChairs) do
			exports['qb-target']:AddBoxZone("BeanChair"..k, v.location, v.width, v.depth, { name="BeanChair"..k, heading = v.heading, debugPoly=Config.Debug, minZ = v.minZ, maxZ = v.maxZ, }, 
				{ options = { { event = "nc-beanmachine:Chair", icon = "fas fa-chair", label = "Sit Down", loc = v.location, head = v.heading, seat = v.seat }, }, distance = v.distance })
		end
	end
...
RegisterNetEvent('nc-beanmachine:Chair', function(data)
	local canSit = true
	local sitting = false
