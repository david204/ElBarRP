...
RegisterNetEvent("nc-recycle:removeWarehouseProps")
AddEventHandler("nc-recycle:removeWarehouseProps", function()
    CleanUpArea()
end)
...
Citizen.CreateThread(function()
    ...
    { options = { { event = "expand:recyling:EnterTradeWarehouse", icon = "fas fa-recycle", label = "Enter recycling place", }, },
					distance = 1.5
    })
    ...
    { options = { { event = "expand:recyling:ExitTradeWarehouse", icon = "fas fa-recycle", label = "Exit recycling place", }, },
					distance = 1.5
    })
    ...
    { options = { { event = "nc-recycle:Trade:Menu", icon = "fas fa-box", label = "Trade Materials", }, },
					distance = 1.5
    })
    exports['qb-target']:AddCircleZone("sellmats", Config.Locations['Recycle'].location, 2.0, { name="sellmats", debugPoly=false, useZ=true, },
    { options = { { event = "nc-recycle:Selling:Menu", icon = "fas fa-box", label = "Sell Materials", }, },
					distance = 2.5
    })
	--Dumpster Third Eye
	exports['qb-target']:AddTargetModel(dumpsters, { options = { { event = "nc-recycle:Dumpsters:Search", icon = "fas fa-dumpster", label = "Search Trash", }, },
					distance = 1.5
    })
    ...
    { options = { { event = "nc-recycle:Bottle:Menu", icon = "fas fa-certificate", label = "Sell Bottles", },	},
					distance = 2.5
    })
...
AddEventHandler('expand:recyling:ExitTradeWarehouse', function()
	TriggerEvent('nc-recycle:removeWarehouseProps')
	DoScreenFadeOut(500)
	while not IsScreenFadedOut() do
		Citizen.Wait(10)
	end
                    ...
                    if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, Config['delivery'].DropLocation.x, Config['delivery'].DropLocation.y, Config['delivery'].DropLocation.z, true) < 2.0 then
                        ...
                        if IsControlJustReleased(0, 38) then
                            ...
                            QBCore.Functions.Progressbar("deliver_reycle_package", "Packing into recyclable box..", 5000, false, true, {
                                ...
                                TriggerServerEvent('nc-recycle:getrecyclablematerial')
                                GetRandomPackage()
                            end)
                        end
                    else
...
RegisterNetEvent('nc-recycle:SellAnim')
AddEventHandler('nc-recycle:SellAnim', function(data)
	if data == -2 then
	...
	if data == 1 then
		TriggerServerEvent('nc-recycle:Selling:All')
	else
		TriggerServerEvent('nc-recycle:Selling:Mat', data)
	end
...
--Sell Anim small Test
RegisterNetEvent('nc-recycle:TradeAnim')
AddEventHandler('nc-recycle:TradeAnim', function(data)
	local pid = PlayerPedId()
	loadAnimDict("mp_common")
	TriggerServerEvent('nc-recycle:TradeItems', data) -- Had to slip in the sell command during the animation command
	for k,v in pairs (shopPeds) do
        pCoords = GetEntityCoords(PlayerPedId())
        ppCoords = GetEntityCoords(v)
		ppRot = GetEntityRotation(v)
...
--Material Buyer
RegisterNetEvent('nc-recycle:Selling:Menu', function()
    exports['qb-menu']:openMenu({
		{ header = "Material Selling", txt = "Sell batches of materials", isMenuHeader = true },
		{ header = "", txt = "✘ Close", params = { event = "nc-recycle:SellAnim", args = -2 } },
		{ header = QBCore.Shared.Items["copper"].label, params = { event = "nc-recycle:SellAnim", args = 'copper' } },
		{ header = QBCore.Shared.Items["plastic"].label, params = { event = "nc-recycle:SellAnim", args = 'plastic' } },
		{ header = QBCore.Shared.Items["aluminum"].label, params = { event = "nc-recycle:SellAnim", args = 'aluminum' } },
		{ header = QBCore.Shared.Items["metalscrap"].label, params = { event = "nc-recycle:SellAnim", args = 'metalscrap' }  },
		{ header = QBCore.Shared.Items["steel"].label, params = { event = "nc-recycle:SellAnim", args = 'steel' } },
		{ header = QBCore.Shared.Items["glass"].label, params = { event = "nc-recycle:SellAnim", args = 'glass' } },
		{ header = QBCore.Shared.Items["iron"].label, params = { event = "nc-recycle:SellAnim", args = 'iron' } },
		{ header = QBCore.Shared.Items["rubber"].label, params = { event = "nc-recycle:SellAnim", args = 'rubber' } },
		{ header = "ALL", params = { event = "nc-recycle:SellAnim", args = 1 } }, 
    })
end)

--Recyclable Trader
RegisterNetEvent('nc-recycle:Trade:Menu', function()
    exports['qb-menu']:openMenu({
		{ header = "Material Trading", txt = "Trade collected materials", isMenuHeader = true },
 		{ header = "", txt = "✘ Close", params = { event = "nc-recycle:SellAnim", args = -2 } },
		{ header = "Trade 10 Materials", params = { event = "nc-recycle:TradeAnim", args = 1 } },
		{ header = "Trade 100 Materials", params = { event = "nc-recycle:TradeAnim", args = 2 } },
    })
end)

--Recyclable Trader
RegisterNetEvent('nc-recycle:Bottle:Menu', function()
    exports['qb-menu']:openMenu({
		{ header = "Material Selling", txt = "Sell batches of recyclables", isMenuHeader = true },
		{ header = "", txt = "✘ Close", params = { event = "nc-recycle:SellAnim", args = -2 } },
		{ header = "Sell "..QBCore.Shared.Items["bottle"].label, params = { event = "nc-recycle:SellAnim", args = 'bottle' } },
		{ header = "Sell "..QBCore.Shared.Items["can"].label, params = { event = "nc-recycle:SellAnim", args = 'can' } },
    })
end)
...
RegisterNetEvent('nc-recycle:Dumpsters:Remove')
AddEventHandler('nc-recycle:Dumpsters:Remove', function(object)
    for i = 1, #searched do
...
RegisterNetEvent('nc-recycle:Dumpsters:Search')
AddEventHandler('nc-recycle:Dumpsters:Search', function()
    local ped = PlayerPedId()
    local playerPed = PlayerPedId()
    --while true do
        local wait = 1000
                    ...
                    for i = 1, #searched do
                        ...
                        elseif i == #searched and not dumpsterFound then
							...
							Skillbar.Start({
                                ...
                                startSearching(searchTime, 'amb@prop_human_bum_bin@base', 'base', 'nc-recycle:Dumpsters:Reward')
