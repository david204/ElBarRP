...
RegisterNetEvent('QBCore:Client:OnPlayerLoaded', function()
    ...
    onDuty = true
    TriggerServerEvent('nc-carboost:server:getItem')
    TriggerEvent('nc-carboost:client:setupLaptop')
end)
...
RegisterNetEvent('QBCore:Client:OnPlayerUnload', function ()
    PlayerJob = {}
    TriggerServerEvent('nc-carboost:server:removeData', PlayerData.citizenid)  
    PlayerData = {}
    Config.BennysItems = {}
end)
...
local function finishBoosting(data)
    TriggerServerEvent('nc-carboost:server:finishBoosting', 'normal', Tier)
    TriggerEvent('nc-carboost:client:deleteContract')
    ...
    ContractID = nil
    Wait(1000)
    TriggerEvent('nc-carboost:client:refreshQueue')
end

local function Scratching()
    TriggerServerEvent('nc-carboost:server:finishBoosting', 'vin', Tier)
    TriggerEvent('nc-carboost:client:deleteContract')
...
local function BoostingAlert()
    if Config.Alert == 'nc-dispatch' then
        AlertBoosting(carID, 'nc-dispatch')
...
RegisterNUICallback('wallpaper', function (data)
    ...
    TriggerServerEvent('nc-carboost:server:saveConfig', data)
...
RegisterNUICallback('loadstore', function (data, cb)
    ...
    if Config.BennysSell then
                ...
                if QBCore.Shared.Items[v.item] ~= nil then
                    if Config.BennysSell[k].name then
                       ...
                       if QBCore.Shared.Items[v.item] ~= nil then
                        ...
                        name = "Unknown"
                         return TriggerServerEvent('nc-carboost:server:log', "")
                       end
                    end
                    ...
                    TriggerServerEvent('nc-carboost:server:log', "The item is not found :"..k)
                end
            else
                TriggerServerEvent("nc-carboost:server:log", "Duplicate item found: " .. k)
            end
        end
        cb({
            storeitem = storeitem
        ...
        TriggerServerEvent('nc-carboost:server:log', "The store is empty")
        cb({
...
RegisterNUICallback('canStartContract', function (data, cb)
    ...
    if isContractStarted then
        ...
        if data.type == 'vin' then
            QBCore.Functions.TriggerCallback('nc-carboost:server:vinmoney', function(result)
...
RegisterNUICallback('startcontract', function (data)
    ...
    if not isContractStarted then
        ...
        Tier = data.data.tier
        QBCore.Functions.TriggerCallback('nc-carboost:server:getContractData', function (result)
            if result then
                TriggerEvent('nc-carboost:client:spawnCar', result)
            else
                print("NO RESULT")
            end
        end, data)
...
RegisterNUICallback('stopcontract', function (data)
    if isContractStarted then
        isContractStarted = false
        TriggerEvent('nc-carboost:client:stopContract')
    end
end)

RegisterNUICallback('checkout', function (data)
    QBCore.Functions.TriggerCallback('nc-carboost:server:canBuy', function (result)
        if result then
            ...
            TriggerServerEvent('nc-carboost:server:buyItem', itemData.price, Config.BennysItems, firstTime)
            SendNUIMessage({
                type="checkout",
                success = result
            })
...
RegisterNUICallback('setupboostapp', function (data, cb)
    QBCore.Functions.TriggerCallback('nc-carboost:server:getboostdata', function (result)
        if result then
...
RegisterNUICallback('sellcontract', function(data, cb)
    QBCore.Functions.TriggerCallback('nc-carboost:server:sellContract', function (result)
        if result then
            cb(result)
        else
            cb({
...
RegisterNUICallback('transfercontract', function (data, cb)
    QBCore.Functions.TriggerCallback('nc-carboost:server:transfercontract', function (result)
        if result and not result.error then
            cb({
                success = result
            })
...
RegisterNUICallback('joinqueue', function (data)
    TriggerEvent('nc-carboost:client:joinQueue', data)
end)

RegisterNUICallback('buycontract', function(data, cb)
    QBCore.Functions.TriggerCallback('nc-carboost:server:buycontract', function(result)
        if result then
            cb(result)
        end
    end, data)
...
RegisterNetEvent('nc-carboost:client:setConfig', function (data)
    Config.BennysItems = data
end)

RegisterNetEvent('nc-carboost:client:joinQueue', function (data)
    isJoinQueue = data.status
    local citizenid = PlayerData.citizenid
    TriggerServerEvent('nc-carboost:server:joinQueue', isJoinQueue, citizenid)
end)

RegisterNetEvent('nc-carboost:client:refreshQueue', function ()
    local citizenid = PlayerData.citizenid
    if isJoinQueue then
        TriggerServerEvent('nc-carboost:server:joinQueue', false, citizenid)
        Wait(2000)
        TriggerServerEvent('nc-carboost:server:joinQueue', true, citizenid)
    end
end)

RegisterNetEvent('nc-carboost:client:giveContract', function ()
    SendNUIMessage({
        type="givecontract",
        data = {
...
RegisterNetEvent('nc-carboost:client:setupLaptop', function ()
    TriggerEvent('nc-carboost:client:sendLaptopConfig')
    TriggerEvent('nc-carboost:client:setupBoostingApp')
    TriggerServerEvent('nc-carboost:server:getBoostSale')
end)

RegisterNetEvent('nc-carboost:client:setupBoostingApp', function ()
    SendNUIMessage({
        type="setupboostingapp",
    })
end)

RegisterNetEvent('nc-carboost:client:sendLaptopConfig', function ()
...
RegisterNetEvent('nc-carboost:client:newContract', function ()
    TriggerServerEvent('nc-carboost:server:newContract', PlayerData.citizenid)
end)

RegisterNetEvent('nc-carboost:client:addContract', function (data)
...
RegisterNetEvent('nc-carboost:client:takeAll', function ()
    TriggerServerEvent('nc-carboost:server:takeAll', Config.BennysItems)
    Config.BennysItems = {}
end)

RegisterNetEvent('nc-carboost:client:takeItem', function (data)
    TriggerServerEvent('nc-carboost:server:takeItem', data.item, data.quantity)
    ...
    TriggerServerEvent('nc-carboost:server:updateBennysConfig', Config.BennysItems)
end)

RegisterNetEvent('nc-carboost:client:openMenu', function ()
    if Config.BennysItems[1] then
        ...
        for _, v in pairs(Config.BennysItems) do
            ...
            menu[#menu+1] = {
                ...
                params = {
                    event = "nc-carboost:client:takeItem",
                    args = {
                            item = item.name,
        ...
        menu[#menu+1] = {
            ...
            params = {
                event = "nc-carboost:client:takeAll"
            }
...
RegisterNetEvent('nc-carboost:client:spawnCar', function(data)
    QBCore.Functions.TriggerCallback('nc-carboost:server:spawnCar', function(result)
        if result then
            ...
            TriggerServerEvent('qb-phone:server:sendNewMail', {
             ...
             TriggerEvent('nc-carboost:client:startBoosting', result)
        end
    end, data)
end)

RegisterNetEvent('nc-carboost:client:startBoosting', function (data)
    local data = data
    ContractID = data.id
                ...
                if DoesEntityExist(NetworkGetEntityFromNetworkId(carID)) then
                    ...
                    if dist <= 5.0 then
                        if blipDisplay ~= nil then
                            ...
                            TriggerEvent('nc-carboost:client:playerInVehicle', data)
                            break
                        end
                    end
                end
...
RegisterNetEvent('nc-carboost:client:checkvin', function ()
        ...
        if #(PlayerPos - vehPos) <= 5.0 then
            ...
            if GetVehicleDoorLockStatus(vehicle) == 1 then
                    QBCore.Functions.Progressbar('check_vin', 'Checking VIN Number', 6000, false, true, { -- Name | Label | Time | useWhileDead | canCancel
                        ...
                        QBCore.Functions.TriggerCallback('nc-carboost:server:checkvin', function(result)
...
RegisterNetEvent('nc-carboost:client:vinscratch', function(veh)
    ...
    QBCore.Functions.Progressbar('vin_scratching', 'Scratching VIN', 7000, false, true, { -- Name | Label | Time | useWhileDead | canCancel
        ...
        QBCore.Functions.TriggerCallback('nc-carboost:server:checkvin', function(result)
            if result and result.owner == PlayerData.citizenid then
                ...
                TriggerServerEvent('nc-carboost:server:vinscratch', ID, vehProp, carmodel)
                TriggerEvent('nc-carboost:client:deleteContract')
                Scratching()
            end
        end, ID)
...
RegisterNetEvent('nc-carboost:client:playerInVehicle', function (data)
    local data = data
    CreateThread(function ()
        while true do
            Wait(100)
            if IsPedInVehicle(PlayerPedId(), carSpawned, false) then
                if IsVehicleAlarmActivated(carSpawned) or IsVehicleEngineStarting(carSpawned) or GetIsVehicleEngineRunning(carSpawned) then       
                    ...
                    TriggerEvent('nc-carboost:client:trackersReady', data)
                    break
                end
            end
        end
...
RegisterNetEvent('nc-carboost:client:trackersReady', function (data)
    CreateThread(function ()
        while true do
            Wait(1000)
            if GetIsVehicleEngineRunning(carSpawned) then
                ...
                BoostingAlert()
                TriggerEvent('nc-carboost:client:startTracker', data)
                break
            end
        end
    end)
...
RegisterNetEvent('nc-carboost:client:startTracker', function(data)
    local veh = Entity(carSpawned)
    CreateThread(function ()
        while true do
            Wait(5000)
            if not veh.state.tracker then
                TriggerEvent("nc-carboost:client:bringtoPlace", data)
                break
            end
        end
    end)
...
RegisterNetEvent("nc-carboost:client:bringtoPlace", function (data)
...
RegisterNetEvent('nc-carboost:client:finishBoosting', function (data)

end)

RegisterNetEvent('nc-carboost:client:updateProggress', function (data)
    local data = {
...
RegisterNetEvent('nc-carboost:client:deleteContract', function ()
    SendNUIMessage({
        type = "removeContract",
        id = ContractID
    })
    TriggerServerEvent('nc-carboost:server:deleteContract', ContractID)
end)

RegisterNetEvent('nc-carboost:client:playerUnload', function()
    if carSpawned then
        DeleteVehicle(carSpawned)
    end
    TriggerServerEvent('nc-carboost:server:joinQueue', false, PlayerData.citizenid)
end)

RegisterNetEvent('nc-carboost:client:failedBoosting', function ()
    if zone then
        zone:destroy()
        zone = nil
        inZone = false
...
RegisterNetEvent('nc-carboost:client:openLaptop', function ()
    SetDisplay(not display)
end)

RegisterNetEvent('nc-carboost:client:loadBoostStore', function(data)
    local cdata = data
    local saleData = {}
...
RegisterNetEvent('nc-carboost:client:refreshContract', function ()
    QBCore.Functions.TriggerCallback('nc-carboost:server:getContractData')
    SendNUIMessage({
        type = "refreshContract",
    })
end)

RegisterNetEvent('nc-carboost:client:newContractSale', function(data)
    local data = data 
...
RegisterNetEvent('nc-carboost:client:contractBought', function(id)
    SendNUIMessage({
        type = "contractbought",
        id = id
    })
...
RegisterNetEvent('nc-carboost:client:useHackingDevice', function ()
...
RegisterNetEvent('nc-carboost:client:fakeplate', function()
                ...
                if result then
                    if result.owner == PlayerData.citizenid then
                        QBCore.Functions.Progressbar('change_plate', 'Changing the plate', 8000, false, true, { -- Name | Label | Time | useWhileDead | canCancel
                            ...
                            TriggerEvent('nc-carboost:client:setPlate', vehID)
                            ClearPedTasks(PlayerPedId())
                        end, function() -- Play When Cancel
                            ClearPedTasks(PlayerPedId())
                            --Stuff goes here
...
RegisterNetEvent('nc-carboost:client:setPlate', function (vehID)
    local veh = NetworkGetEntityFromNetworkId(vehID)
    if veh and veh ~= 0 then
        local plate = QBCore.Functions.GetPlate(veh)
        local newPlate = RandomPlate()
        SetVehicleNumberPlateText(veh, newPlate)
        TriggerServerEvent('nc-carboost:server:setPlate', plate, newPlate)
    ...
    while true do
        ...
        if carSpawned ~= nil and not DoesEntityExist(carSpawned) then
            if carSpawned ~= 0 then
                TriggerEvent('nc-carboost:client:failedBoosting')
            end
        end
    end
end)
...
exports['qb-target']:AddBoxZone("carboost:takeItem", vector3(1185.14, -3304.01, 7.1), 2, 2, {
	...
	options = {
		{
            type = "client",
            event = "nc-carboost:client:openMenu",
    ...
    options = {
        {
            ...
            action = function (entity)
                TriggerEvent('nc-carboost:client:vinscratch', entity)
            end
        },
    },
    distance = 1.2
