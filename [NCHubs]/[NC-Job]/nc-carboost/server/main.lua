...
AddEventHandler('onResourceStart', function (resource)
   if resource == GetCurrentResourceName() then
      ...
      GenerateVIN()
      PerformHttpRequest('https://raw.githubusercontent.com/JustLazzy/nc-carboost/master/version', CheckVersion, 'GET')
   end
end)
...
-- Event
RegisterNetEvent('nc-carboost:server:saveBoostData', function (citizenid)
...
RegisterNetEvent('nc-carboost:server:newContract', function (source)
   local src = source
   ...
   if #Config.QueueList[citizenid].contract <= Config.MaxContract then     
      MySQL.Async.insert('INSERT INTO boost_contract (owner, data, started, expire) VALUES (@owner, @data, NOW(),DATE_ADD(NOW(), INTERVAL @expire HOUR))', {
         ...
         TriggerClientEvent('nc-carboost:client:addContract', src, contractData)
      end)
   end
end)

RegisterNetEvent('nc-carboost:server:joinQueue', function (status, citizenid)
   local src = source
   local Player = QBCore.Functions.GetPlayer(src)
   if status then
      ...
      TriggerEvent('nc-carboost:server:log', 'Player with CID: '..citizenid..' joined the queue, queue number: '..queueNumber)
      TriggerEvent('nc-carboost:server:getContract', Player.PlayerData.citizenid)
   else
      Config.QueueList[citizenid] = {
         status =  false,
         tier = Player.PlayerData.metadata['carboostclass'],
      ...
      TriggerEvent('nc-carboost:server:log', 'Player with CID: '..citizenid..' has left the queue, queue number: '..queueNumber)
   end
end)

-- This one is only for serverside
RegisterNetEvent('nc-carboost:server:getContract', function (citizenid)
...
RegisterNetEvent('nc-carboost:server:takeItem', function (name, quantity)
   local src = source
   local Player = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent('nc-carboost:server:getItem', function ()
   local src = source
   ...
   if result then
      TriggerClientEvent('nc-carboost:client:setConfig', src, json.decode(result))
   end
end)

RegisterNetEvent('nc-carboost:server:setConfig', function ()
   TriggerClientEvent('nc-carboost:client:setConfig', -1, Config.BennysItems)
end)

RegisterNetEvent('nc-carboost:server:giveContract', function ()
   local src = source
   local pData = QBCore.Functions.GetPlayer(src)
   TriggerEvent('nc-carboost:server:newContract', src,pData.PlayerData.citizenid)
end)

RegisterNetEvent('nc-carboost:server:buyItem', function (price, config, first)
   local src = source 
   local pData = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent('nc-carboost:server:log', function (string, type)
...
RegisterNetEvent('nc-carboost:server:finishBoosting', function (type, tier)
   ...
   TriggerClientEvent('nc-carboost:client:updateProggress', src, isNextLevel)
   TriggerClientEvent('QBCore:Notify', src, Lang:t('info.get_rep', {
      rep = randomRep
   }), "primary")   
end)

RegisterNetEvent('nc-carboost:server:saveConfig', function (data)
...
RegisterNetEvent('nc-carboost:server:deleteContract', function (contractid)
   local src = source
   local Player = QBCore.Functions.GetPlayer(src)
   MySQL.Async.execute('DELETE FROM boost_contract WHERE owner = @citizenid AND id = @id',{
      ...
      if result > 0 then
         TriggerEvent('nc-carboost:server:log', 'Contract '..contractid..' deleted, CID:'..Player.PlayerData.citizenid)
      end
   end)
   print("END OF THE DELETE CONTRACT")
end)

RegisterNetEvent('nc-carboost:server:updateBennysConfig', function (data)
   local src = source 
   local pData = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent('nc-carboost:server:vinscratch', function(NetworkID, mods, model)
   local src = source 
   local pData = QBCore.Functions.GetPlayer(src)
...
QBCore.Commands.Add('giveContract', 'Give contract, admin only', {
   ...
   if Config.Tier[tostring(args[1])] ~= nil then
      ...
      MySQL.Async.insert('INSERT INTO boost_contract (owner, data, started, expire) VALUES (@owner, @data, NOW(),DATE_ADD(NOW(), INTERVAL @expire HOUR))', {
         ...
         TriggerClientEvent('nc-carboost:client:addContract', player.PlayerData.source, contractData)
...
RegisterNetEvent('nc-carboost:server:takeAll', function (data)
   local src = source
   local Player = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent('nc-carboost:server:getBoostSale', function()
   local src = source
   local result = MySQL.Sync.fetchAll('SELECT * FROM boost_contract WHERE onsale = 1')
   if result[1] then
      TriggerClientEvent('nc-carboost:client:loadBoostStore', src,result)
   end
end)

RegisterNetEvent('nc-carboost:server:setPlate', function (oldPlate, newPlate)
...
-- Alert
RegisterNetEvent('nc-carboost:notifyboosting', function (pos, car)
   TriggerClientEvent('nc-carboost:notifyboosting', -1, pos, car)
end)

RegisterNetEvent('nc-carboost:notifypolice', function (car)
   TriggerClientEvent('nc-carboost:notifypolice', -1, car)
end)

-- Callback
QBCore.Functions.CreateCallback('nc-carboost:server:canBuy', function(source, cb, data)
   local src = source
   local pData = QBCore.Functions.GetPlayer(src)
...
QBCore.Functions.CreateCallback('nc-carboost:server:sellContract', function (source, cb, data)
   ...
   if result[1] then
      ...
      TriggerClientEvent('nc-carboost:client:newContractSale', -1, contractData)
      return cb({
...
QBCore.Functions.CreateCallback('nc-carboost:server:buycontract', function(source, cb, data)
   local src = source
   local Player = QBCore.Functions.GetPlayer(src)
   local toPlayer
   ...
   if result[1] then
      ...
      TriggerClientEvent('nc-carboost:client:contractBought', -1, data.id)
      local contractData = json.decode(contractInfo.data)
...
QBCore.Functions.CreateCallback('nc-carboost:server:transfercontract', function (source, cb, data)
   local src = source
   ...
   if result[1] then
      MySQL.Async.execute('UPDATE boost_contract SET owner = @owner WHERE owner = @citizenid', {
         ...
         TriggerClientEvent('nc-carboost:client:addContract', toPlayer.PlayerData.source, contractData)
         return cb({
            success = true
         })
      end)
...
QBCore.Functions.CreateCallback('nc-carboost:server:vinmoney', function (source, cb, data)
   local src = source
   local Player = QBCore.Functions.GetPlayer(src)
   local money = Player.PlayerData.money[Config.Payment]
...
QBCore.Functions.CreateCallback('nc-carboost:server:getboostdata', function (source, cb, citizenid)
...
QBCore.Functions.CreateCallback('nc-carboost:server:getContractData', function (source, cb, data)
...
QBCore.Functions.CreateCallback('nc-carboost:server:spawnCar', function (source, cb, data)
   local src = source
   local cardata = data
...
QBCore.Functions.CreateCallback('nc-carboost:server:checkvin', function (source, cb, data)
   local src = source
   ...
   if queueNumber ~= 0 then
      ...
      for k, v in pairs(Config.QueueList) do
         ...
         if not v.getContract and v.status and Player and num <= Config.MaxQueueContract then
            ...
            TriggerEvent('nc-carboost:server:newContract', Player.PlayerData.source, k)
            num = (num or 0) + 1
            v.getContract = true
         end
...
--    -- https://raw.githubusercontent.com/JustLazzy/nc-carboost/master/version
--    local curVersion = LoadResourceFile(GetCurrentResourceName(), 'version')
--    if curVersion ~= resp and tonumber(curVersion) < tonumber(resp) then
--       print('[nc-carboost] New version available: ' .. resp)
--    else
--       print('[nc-carboost] No new version available')
--    end
-- end
...
RegisterNetEvent('nc-testing', function ()
...
QBCore.Functions.CreateUseableItem('pixellaptop' , function(source, item)
   TriggerClientEvent('nc-carboost:client:openLaptop', source)
end)

QBCore.Functions.CreateUseableItem('hacking_device',  function (source, item)
   TriggerClientEvent('nc-carboost:client:useHackingDevice', source)
end)

QBCore.Functions.CreateUseableItem('fake_plate', function(source, item)
   TriggerClientEvent('nc-carboost:client:fakeplate', source)
end)