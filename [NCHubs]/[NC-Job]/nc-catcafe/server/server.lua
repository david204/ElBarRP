...
CreateThread(function()
    ...
    for k,v in pairs(food) do QBCore.Functions.CreateUseableItem(v, function(source, item) TriggerClientEvent('nc-catcafe:client:Eat', source, item.name) end) end
	
	local drinks = { "bobatea", "bbobatea", "gbobatea", "pbobatea", "obobatea", "nekolatte", "mocha" }
    for k,v in pairs(drinks) do QBCore.Functions.CreateUseableItem(v, function(source, item) TriggerClientEvent('nc-catcafe:client:Drink', source, item.name) end) end
	
	local alcohol = { "sake" }
    for k,v in pairs(alcohol) do QBCore.Functions.CreateUseableItem(v, function(source, item) TriggerClientEvent('nc-catcafe:client:DrinkAlcohol', source, item.name) end) end
end)

RegisterServerEvent('nc-catcafe:GetFood', function(data)
	local src = source
...
QBCore.Functions.CreateCallback('nc-catcafe:get', function(source, cb, item, tablenumber, craftable)
	local src = source
	local hasitem = nil
