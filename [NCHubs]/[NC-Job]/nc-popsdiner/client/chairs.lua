...
CreateThread(function()
	for k, v in pairs(Config.Chairs) do
		exports['qb-target']:AddBoxZone("PopChair"..k, v.location, v.width, v.height, { name="PopChair"..k, heading = v.heading, debugPoly=Config.Debug, minZ = v.minZ, maxZ = v.maxZ, }, 
			{ options = { { event = "nc-popsdiner:Chair", icon = "fas fa-chair", label = "Sit Down", loc = v.location, head = v.heading, seat = v.seat }, },
			  distance = v.distance
		})
	end
end)

RegisterNetEvent('nc-popsdiner:Chair', function(data)
	local canSit = true
	local sitting = false
