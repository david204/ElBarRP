...
document.onkeydown = (e) =>{
    ...
    if (key == "Escape"){
        ...
        $.post("http://nc-10system/closeMenu",JSON.stringify({}));
    }
};
...
$(document).on("click","#savesettings",function(){
    ...
    if (callsign){
        $.post("http://nc-10system/saveChanges",JSON.stringify({
            callsign
        }));   
    }
})
...
$(document).on("click", ".enable_list", function(){
    ...
    $.post("http://nc-10system/toggleList", JSON.stringify({
        toggled
    }));
})
...
SaveCallSigns = () => {
    ...
    $.post("http://nc-10system/saveTags", JSON.stringify({
        alltags
    }));
}
...
SaveNewCallSigns = () => {
    ...
    $.post("http://nc-10system/saveNewTags", JSON.stringify({
        alltags
    }));
}
