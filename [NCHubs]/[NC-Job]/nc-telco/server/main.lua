...
RegisterServerEvent('nc-telco:server:cWJ0ZWxjbw')
AddEventHandler('nc-telco:server:cWJ0ZWxjbw', function(TaskDones)
    local src = source 
...
RegisterServerEvent('nc-telco:server:SuretyBond')
AddEventHandler('nc-telco:server:SuretyBond', function(bool, vehInfo)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    ...
    if bool then
        if Player.PlayerData.money.cash >= Config.BailPrice then
            ...
            TriggerClientEvent('nc-telco:client:SpawnVehicle', src, vehInfo)
