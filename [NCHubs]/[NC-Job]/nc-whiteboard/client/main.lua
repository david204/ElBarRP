...
Citizen.CreateThread(function()
    ...
    -- Sync Image
    QBCore.Functions.TriggerCallback('nc-whiteboard:getCurrentImage', function(images)
    ...
    for k,v in pairs(Config.Locations) do
        ...
        exports['qb-target']:AddBoxZone(k..'_Target', v.Target.coords, v.Target.length, v.Target.width, {
            ...
            options = {
                {
                    type = "client",
                    event = "nc-whiteboard:changewhiteboardurl",
                    location = k,
                    icon = "fa fa-camera",
                    label = "Change Image",
                    job = v.job
...
AddEventHandler("nc-whiteboard:changewhiteboardurl", function(data)
    if data.url then
        TriggerServerEvent("nc-whiteboard:changewhiteboard", data.url, data.location)
    else
        ...
        if link then
            TriggerServerEvent("nc-whiteboard:changewhiteboard", link, data.location)
        end
    end
end)

RegisterNetEvent("nc-whiteboard:changewhiteboardcli", function(pUrl, pRoom)
