...
CreateThread(function()
    for k, floshop in pairs(Config.Locations["floshop"]) do
        local blip = AddBlipForCoord(floshop.coords.x, floshop.coords.y, floshop.coords.z)
        SetBlipSprite(blip, 351)
        SetBlipAsShortRange(blip, true)
        SetBlipScale(blip, 0.6)
        SetBlipColour(blip, 43)
        BeginTextCommandSetBlipName("STRING")
        AddTextComponentString(floshop.label)
        EndTextCommandSetBlipName(blip)
    end
end)

CreateThread(function()
    for k, floprocess in pairs(Config.Locations["floprocess"]) do
        local blip = AddBlipForCoord(floprocess.coords.x, floprocess.coords.y, floprocess.coords.z)
        SetBlipSprite(blip, 365)
        SetBlipAsShortRange(blip, true)
        SetBlipScale(blip, 0.6)
        SetBlipColour(blip, 36)
        BeginTextCommandSetBlipName("STRING")
        AddTextComponentString(floprocess.label)
        EndTextCommandSetBlipName(blip)
    end
end)

-------------------------flower-------------------------
RegisterNetEvent("nc-flowerjob:client:flowerpick")
AddEventHandler("nc-flowerjob:client:flowerpick", function ()
    QBCore.Functions.TriggerCallback('nc-flowerjob:server:get:bucket', function(hasItem)
        if hasItem then
            QBCore.Functions.Progressbar('search_flower', Config.ProcessName['pickflower'], Config.ProcessTime['pickflower'], false, true, {
                ...
                TriggerServerEvent("nc-flowerjob:server:flowerpick")
...
-----------------------flower--process--------------------------
RegisterNetEvent("nc-flowerjob:client:processflo")
AddEventHandler("nc-flowerjob:client:processflo", function ()
    QBCore.Functions.TriggerCallback('nc-flowerjob:server:get:processflo', function(hasItem)
        if hasItem then
            QBCore.Functions.Progressbar('process_flo', Config.ProcessName['proflowers'], Config.ProcessTime['proflowers'], false, true, { 
                ...
                TriggerServerEvent("nc-flowerjob:server:processflo")
            end, function ()
                QBCore.Functions.Notify(Config.Notify['cancel'], "error")
            end)
        elseif not hasItem then
...
--------------------------flower-process-2-----------------------------------
RegisterNetEvent("nc-flowerjob:client:packingflo")
AddEventHandler("nc-flowerjob:client:packingflo", function ()
    QBCore.Functions.TriggerCallback('nc-flowerjob:server:get:packingflo', function (hasItem)
        if hasItem then
            QBCore.Functions.Progressbar('pack', Config.ProcessName['packflowers'], Config.ProcessTime['packflowers'], false, true, { -- Name | Label | Time | useWhileDead | canCancel
                ...
                TriggerServerEvent("nc-flowerjob:server:packingflo")
...
-------------------------------selling--flowers------------------------------
RegisterNetEvent("nc-flowerjob:client:sellflower")
AddEventHandler("nc-flowerjob:client:sellflower", function ()
    QBCore.Functions.TriggerCallback('nc-flowerjob:server:get:sellflower', function (hasItem)
        if hasItem then
            ...
            QBCore.Functions.Progressbar('sell_flower', Config.ProcessName['sellflowers'], Config.ProcessTime['sellflowers'], false, true, { 
                ...
                TriggerServerEvent("nc-flowerjob:server:sellflower")
                ClearPedTasks(playerPed)
            end, function ()
                QBCore.Functions.Notify(Config.Notify['cancel'], "error")
            end)
...
RegisterNetEvent("nc-flowerjob:client:openshop")
AddEventHandler("nc-flowerjob:client:openshop", function ()
    ...
    QBCore.Functions.Progressbar('open_shop', Config.ProcessName['openshop'], Config.ProcessTime['openshop'], false, true, { 
        ...
        TriggerEvent('animations:client:EmoteCommandStart', {"c"})
        TriggerEvent('nc-flowerjob:shop')
        ClearPedTasks(playerPed)
...
RegisterNetEvent("nc-flowerjob:shop")
AddEventHandler("nc-flowerjob:shop", function()
...
Citizen.CreateThread(function ()
    exports['qb-target']:AddBoxZone("flowertree1", vector3(1580.06, 2155.73, 79.48), 5, 2,{
        ...
        options = {
            {
                event = "nc-flowerjob:client:flowerpick",
                icon = "fas fa-circle",
                label = "Pick Flowers",
            },
        },
    ...
    exports['qb-target']:AddBoxZone("process1",vector3(1558.77, 2161.02, 78.68), 1, 1,{
        name = "process1",
        heading = 0,
        debugPoly = false,
        minZ=76.74,
        ...
        options = {
            {
                event = "nc-flowerjob:client:processflo",
                icon = "fas fa-circle",
                label = "Process Flowers",
            },
            {
                event = "nc-flowerjob:client:packingflo",
                icon = "fas fa-circle",
    ...
    exports['qb-target']:AddBoxZone("sell", vector3(1551.02, 2189.8, 78.85), 1, 1, {
        ...
        options = {
            {
                event = "nc-flowerjob:client:sellflower",
                icon = "fas fa-circle",
                label = "Sell Flowers",
            },
        },
    ...
    exports['qb-target']:AddBoxZone("shop", vector3(1553.42, 2155.12, 78.88), 2, 1, {
        name = "shop",
        heading = 270,
        debugPoly = false,
        minZ=75.49,
        ...
        options = {
            {
                event = "nc-flowerjob:client:openshop",
                icon = "fas fa-shop",
                label = "Open Shop",
            },
        },
