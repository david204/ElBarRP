...
RegisterNetEvent('fishing:SkillBar', function(message)
	...
	elseif Config.Skillbar == "nc-skillbar" then
...
RegisterNetEvent('fishing:client:useFishingBox', function(BoxId)
	TriggerServerEvent("inventory:server:OpenInventory", "stash", 'FishingBox_'..BoxId, {maxweight = 250000, slots = 5})
...
RegisterNetEvent('nc:client:ReturnBoat', function(args)
	local ped = PlayerPedId()
	local args = tonumber(args)
...
RegisterNetEvent('nc:client:rentaBoat', function(args)
	local args = tonumber(args)
		...
		if isSuccess then 
			...
			if args == 1 then 
				QBCore.Functions.SpawnVehicle(Config.RentalBoat, function(boat)
					...
					exports['nc-fuel']:SetFuel(boat, 100.0)
...
RegisterNetEvent('nc:client:BoatMenu', function(data)
	local ped = PlayerPedId()
