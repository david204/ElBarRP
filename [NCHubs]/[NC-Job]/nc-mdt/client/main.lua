...
RegisterNetEvent('QBCore:Client:OnPlayerUnload', function()
    TriggerServerEvent("nc-mdt:server:OnPlayerUnload")
    PlayerData = {}
end)
...
RegisterNetEvent("QBCore:Client:SetDuty", function(job, state)
    if AllowedJob(job) then
        TriggerServerEvent("nc-mdt:server:ToggleDuty")
    end
end)
...
RegisterNetEvent('mdt:client:setRadio', function(radio)
    if type(tonumber(radio)) == "number" then
        exports["nc-voice"]:setVoiceProperty("radioEnabled", true)
        exports["nc-voice"]:setRadioChannel(tonumber(radio))
