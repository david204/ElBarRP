...
let stressColor  = localStorage.getItem('pstresscolor') ? localStorage.getItem('pstresscolor') : localStorage.setItem('pstresscolor', undefined) ? localStorage.getItem('pstresscolor') : localStorage.getItem('pstresscolor') ;
...
let keybindShow  = localStorage.getItem('showkeybind') ? localStorage.getItem('showkeybind') : localStorage.setItem('showkeybind', "show") ? localStorage.getItem('showkeybind') : localStorage.getItem('showkeybind') ;
...
window.addEventListener('message', function (event) {
    ...
    switch (e.type) {
        ...
        case "ADD_NOTIFICATION":
            ...
            break;
        case "UPDATE_STATUS":
            updateStats(e.heal, e.armour, e.hunger, e.thirst, e.stamina, e.weather, e.streetTitle, e.streetName, e.stress);
            break;
        case "PLAYER_INCAR":
            playerInCar = true;
            $('.car-hud').fadeIn(250);
...
document.onkeyup = function(data){
    if (data.key == "Escape"){
        if (!cooldown && !editorMode) {
            $.post('http://wais-hudv4/close', JSON.stringify({}));
        }else if (editorMode) {
            editorMode = false;
    ...
    if (e.target.type == "checkbox") {
        ...
        if (id == "speed-type") {
            ...
            if (value) {
                ...
                cinematicMode(true);
                $.post('http://wais-hudv4/cinematic', JSON.stringify({cinematic: true}));
            }else {
                ...
                cinematicMode(false);
                $.post('http://wais-hudv4/cinematic', JSON.stringify({cinematic: false}));
            }
        }else if (id == "jobitem") {
            if (value) {
...
$('.container').draggable({
    stop : function( event, ui ) {
        if (event.target.dataset.hud == "pstress" || event.target.dataset.hud == "stamina") {

            if (event.target.dataset.hud == "pstress") {
                $('.container[data-hud="stamina"]').attr('style', $(`.container[data-hud="${event.target.dataset.hud}"]`).attr('style'))
            }else if (event.target.dataset.hud == "stamina") {
...
function loaded() {
    ...
    writeDate();
    writeClock();
    $.post('http://wais-hudv4/nuiloaded', JSON.stringify({}));

    sound = new Audio('noBelt.mp3');
    sound.volume = 0.5;
}

function updateStats(pheal, parmor, phunger, pthirst, pstamina, pweather, streetTitle, streetName, pstress) {
    if (pheal > 100) {
        pheal = 100;
    }
    ...
    if (progbarsLoad) {
        ...
        $('#pstress-text').html(`${parseInt(pstress)}`);
        ...
        if (pstamina < 100 && !pweather) {
            if (!stressHided) {
                stressHided = true
                $('.container[data-hud="pstress"]').fadeOut(250);
                $('.container[data-hud="stamina"]').fadeIn(250);
            }
        }else if (pweather) {
            if (!stressHided) {
                stressHided = true
            ...
            if (stressHided) {
                stressHided = false
                $('.container[data-hud="pstress"]').fadeIn(250);
                $('.container[data-hud="stamina"]').fadeOut(250);
            }else {
                $('.container[data-hud="pstress"]').fadeIn(250);
                $('.container[data-hud="stamina"]').fadeOut(250);
            }
...
function renderProgBras() {
    ...
    if (heal && armour && hunger && thirst && stamina && stress) {
        heal.destroy();
        armour.destroy();
        hunger.destroy();
        thirst.destroy();
    ...
    stress = new barTypes[hudType](`.container-prog[data-hud="pstress"]`, {
        strokeWidth: 7,
        easing: 'easeInOut',
        duration: 250,
        color: stressColor == "undefined" ? rootColors.statstress : stressColor,
        trailColor: "#00000000",
        trailWidth: 7,
        svgStyle: {
            width: '100%'
    ...
    $('.container[data-hud="pstress"]').attr('style', `${staminaPosition != "undefined" || staminaPosition != undefined ? staminaPosition : ""}`);
    ...
    $('.container-prog[data-hud="pstress"]').css({
        "background": `linear-gradient(0deg, ${stressColor == "undefined" ? "rgba(197, 7, 7,0.5)" : stressColor+'50'} 0%, rgba(0,0,0,0) 100%)`
    })

    $('.container-icon[data-hud="pstress"]').css({
        "color": `${stressColor == "undefined" ? "rgba(197, 7, 7,1)" : stressColor}`
    })
    ...
    $(`.color[data-name="pstress"]`).css({
        "background": `${stressColor == "undefined" ? "rgba(197, 7, 7, 0.02)" : stressColor}`,
    })
...
function resetDefault() {
    ...
    $('.container-prog[data-hud="pstress"]').css({"background": "linear-gradient(0deg, rgba(197, 7, 7, 0.5) 0%, rgba(0,0,0,0) 100%)"})
    $('.container-icon[data-hud="pstress"]').css({"color": "linear-gradient(0deg, rgba(197, 7, 7,1) 0%, rgba(0,0,0,0) 100%)"})
    ...
    localStorage.setItem('pstresscolor', undefined);
...
function sendClientMapType() {
    $.post('http://wais-hudv4/setMapSelection', JSON.stringify({
        map: mapType
    }));
}
...
function sendSpeedType() {
    $.post('http://wais-hudv4/speedType', JSON.stringify({
        type: speedType
    }));
}
